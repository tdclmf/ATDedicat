                                                ...
                                                const CNotYetVisibleObject& not_yet_visible_object) const
{
	...
	float dwTime = float(pos1.dwTime) / 1000.f - float(pos0.dwTime) / 1000.f;
	if (dwTime > EPS_S)
		velocity = pos1.vPosition.distance_to(pos0.vPosition) / dwTime;
	else
		velocity = pos1.vPosition.distance_to(pos0.vPosition);
                                              ...
                                              float time_delta, float object_velocity, float luminocity) const
{
	...
	extern luabind::functor<float>* Fget_visible_value;
	if (Fget_visible_value)
		return ((*Fget_visible_value)(m_object ? m_object->lua_game_object() : 0, game_object ? game_object->lua_game_object() : 0,
...
void CVisualMemoryManager::update(float time_delta)
{
	...
	if (m_stalker) {
		...
		if (m_object && Actor() && m_object->is_relation_enemy(Actor()))
		{
			xr_vector<CNotYetVisibleObject>::iterator I = std::find_if(
				m_not_yet_visible_objects.begin(),
				m_not_yet_visible_objects.end(),
