...
{
	PROF_EVENT("CInifile szFileName");
...
xrCriticalSection Overridecs, cs;
static void insert_item(CInifile::Sect* tgt, const CInifile::Item& I)
{
	...
	if (*I.first && (I.first.c_str()[0] == '<' || I.first.c_str()[0] == '>')) {
		xrCriticalSection::raii guard(&Overridecs);
...
)
{
	...
	auto MergeParentSet = [](std::vector<std::string>* ParentsBase, std::vector<std::string>* ParentsOverride, bool bIncludeRemovers)
	{
		...
		)
	{
		...
		auto GetParentStrings = [&](std::string SectionName)
		{
			...
			if (Current && bIsCurrentSectionOverride == bOverridesOnly)
			{
				...
				if (SectIt != OutputData->end())
				{
					if (!bIsCurrentSectionOverride)
					{
                        ...
                        Msg("~ [DLTX] Duplicate section '%s' without explicit override marker; applying as override.\n~ [DLTX] Root file: \"%s\"\n~ [DLTX] First section file: \"%s\"\n~ [DLTX] Duplicate file: \"%s\"",
                            *Current->Name,
                            m_file_name,
                            SectionToFilename[std::string(Current->Name.c_str())].c_str(),
                            currentFileName);
					}

					//Overwrite existing override data
					SectIt->second.Data.reserve(Current->Data.size());
	...
	std::function<void(const std::string&, std::vector<std::string>*)> EvaluateSection = [&](const std::string& SectionName, std::vector<std::string>* PreviousEvaluations)
	{
		...
		//Finalize
		if (!bDeleteSectionIfEmpty || !CurrentSecPair.second.Data.empty())
		{
