...
static xr_string lua_debug_value(lua_State* L, int index)
{
	...
	{
	case LUA_TNIL:
		xr_strcpy(buffer, "<nil>");
		break;
	case LUA_TBOOLEAN:
		xr_strcpy(buffer, lua_toboolean(L, index) ? "true" : "false");
		break;
	case LUA_TNUMBER:
		xr_sprintf(buffer, "%g", lua_tonumber(L, index));
		break;
	case LUA_TSTRING:
		xr_strcpy(buffer, lua_tostring(L, index));
		break;
	default:
		xr_sprintf(buffer, "<%s:%p>", lua_typename(L, value_type), lua_topointer(L, index));
		break;
	}
...
static bool fill_story_name_map_from_ini(xr_map<ALife::_STORY_ID, xr_string>& story_name_by_id)
{
	story_name_by_id.clear();

	if (!pGameIni || !pGameIni->section_exist("story_ids"))
		return false;

	LPCSTR key = 0;
	LPCSTR value = 0;
	shared_str temp;
	for (u32 line = 0; pGameIni->r_line("story_ids", line, &key, &value); ++line)
	{
		if (!key || !*key)
			continue;

		temp = pGameIni->r_string_wb("story_ids", key);
		if (!temp.size())
			continue;

		const ALife::_STORY_ID sid = ALife::_STORY_ID(atoi(key));
		if (sid == INVALID_STORY_ID)
			continue;

		story_name_by_id[sid] = xr_string(*temp);
	}

	return !story_name_by_id.empty();
}

static void sync_story_objects_lua_from_alife_once(LPCSTR reason)
{
	static u32 last_attempt_frame = u32(-1);
	static bool logged_once = false;
	if (last_attempt_frame != u32(-1) && (Device.dwFrame - last_attempt_frame) < 64)
		return;

	last_attempt_frame = Device.dwFrame;
	...
	xr_map<ALife::_STORY_ID, xr_string> story_name_by_id;
	if (!fill_story_name_map_from_ini(story_name_by_id))
	{
		Msg("! [STORY_SYNC] failed: empty story_ids map from pGameIni");
		return;
	}

	lua_getglobal(L, "story_objects");
	if (!lua_istable(L, -1))
	{
		lua_pop(L, 1);
		Msg("! [STORY_SYNC] failed: lua global story_objects is not table");
		return;
	}

	lua_getfield(L, -1, "object_id_by_story_id");
	if (!lua_istable(L, -1))
	{
		lua_pop(L, 1);
		lua_newtable(L);
		lua_pushvalue(L, -1);
		lua_setfield(L, -3, "object_id_by_story_id");
	}
	const int object_id_by_story_id_idx = lua_gettop(L);

	lua_getfield(L, -2, "story_id_by_object_id");
	if (!lua_istable(L, -1))
	{
		lua_pop(L, 1);
		lua_newtable(L);
		lua_pushvalue(L, -1);
		lua_setfield(L, -3, "story_id_by_object_id");
	}
	const int story_id_by_object_id_idx = lua_gettop(L);

	u32 resolved = 0;
	u32 resolved_from_ini = 0;
	u32 resolved_from_object_name = 0;
	u32 missing_name = 0;
	u32 forward_write = 0;
	u32 forward_keep = 0;
	u32 forward_replace = 0;
	u32 reverse_write = 0;
	u32 reverse_keep = 0;

	const CALifeStoryRegistry::STORY_REGISTRY& story_registry = sim->story_objects().objects();
	for (CALifeStoryRegistry::STORY_REGISTRY::const_iterator it = story_registry.begin(); it != story_registry.end(); ++it)
	{
		CSE_ALifeDynamicObject* object = it->second;
		if (!object)
			continue;

		const ALife::_STORY_ID sid = it->first;
		xr_string story_name;
		xr_map<ALife::_STORY_ID, xr_string>::const_iterator name_it = story_name_by_id.find(sid);
		if (name_it != story_name_by_id.end())
		{
			story_name = name_it->second;
			++resolved_from_ini;
		}
		else
		{
			LPCSTR object_story_name = object->name_replace();
			if (!object_story_name || !*object_story_name)
				object_story_name = object->name();

			if (object_story_name && *object_story_name)
			{
				story_name = object_story_name;
				++resolved_from_object_name;
			}
		}
		...
		lua_pushstring(L, story_name.c_str());
		lua_gettable(L, object_id_by_story_id_idx);
		bool write_forward = true;
		if (lua_isnumber(L, -1))
		{
			const u16 existing = u16(lua_tointeger(L, -1));
		...
		if (write_forward)
		{
			...
			lua_settable(L, object_id_by_story_id_idx);
			++forward_write;
		}

		lua_pushinteger(L, object_id);
		lua_gettable(L, story_id_by_object_id_idx);
		bool write_reverse = true;
		if (lua_isstring(L, -1))
		{
			LPCSTR existing_story_name = lua_tostring(L, -1);
			if (existing_story_name && !xr_strcmp(existing_story_name, story_name.c_str()))
			{
				write_reverse = false;
				++reverse_keep;
			}
		...
		if (write_reverse)
		{
			lua_pushinteger(L, object_id);
			lua_pushstring(L, story_name.c_str());
			lua_settable(L, story_id_by_object_id_idx);
			++reverse_write;
		}
	}

	lua_pop(L, 1); // story_id_by_object_id
	lua_pop(L, 1); // object_id_by_story_id
	lua_pop(L, 1); // story_objects

	if (!logged_once || forward_write || forward_replace || reverse_write)
	{
		Msg("* [STORY_SYNC] reason=%s story_registry=%u resolved=%u (ini=%u,name=%u) missing_name=%u forward_write=%u forward_keep=%u forward_replace=%u reverse_write=%u reverse_keep=%u",
			reason ? reason : "<none>",
			u32(story_registry.size()),
			resolved,
			resolved_from_ini,
			resolved_from_object_name,
			missing_name,
...
static void dump_story_objects_state_once(LPCSTR reason)
{
	...
	Msg("* [STORY_DUMP] begin reason=%s", reason ? reason : "<none>");

	const CALifeSimulator* sim = ai().get_alife();
	if (!sim)
	{
		Msg("! [STORY_DUMP] ai().get_alife() is null");
	}
	else
	{
		const CALifeStoryRegistry::STORY_REGISTRY& story_registry = sim->story_objects().objects();
		Msg("* [STORY_DUMP] C++ story_registry size=%u", u32(story_registry.size()));
		for (CALifeStoryRegistry::STORY_REGISTRY::const_iterator it = story_registry.begin(); it != story_registry.end(); ++it)
		{
			CSE_ALifeDynamicObject* object = it->second;
			Msg("* [STORY_DUMP][C++] sid=%u obj_id=%u section=%s name=%s",
				u32(it->first),
				object ? u32(object->ID) : u32(-1),
				object ? object->name() : "<null>",
				object ? object->name_replace() : "<null>");
		}

		const CALifeObjectRegistry::OBJECT_REGISTRY& all_objects = sim->objects().objects();
		u32 with_story_id = 0;
		for (CALifeObjectRegistry::OBJECT_REGISTRY::const_iterator it = all_objects.begin(); it != all_objects.end(); ++it)
		{
			CSE_ALifeDynamicObject* object = it->second;
			...
			++with_story_id;
			Msg("* [STORY_DUMP][OBJ] sid=%u obj_id=%u section=%s name=%s",
				u32(object->m_story_id),
				u32(object->ID),
				object->name(),
				object->name_replace());
		}
		Msg("* [STORY_DUMP] C++ objects_with_story_id=%u", with_story_id);
	}

	lua_State* L = ai().script_engine().lua();
	if (!L)
	{
		Msg("! [STORY_DUMP] lua state is null");
		Msg("* [STORY_DUMP] end");
		return;
	}
	...
	if (!lua_istable(L, -1))
	{
		Msg("! [STORY_DUMP] lua global story_objects is not table");
		lua_pop(L, 1);
		Msg("* [STORY_DUMP] end");
		return;
	}

	{
		...
		{
			xr_string key = lua_debug_value(L, -2);
			xr_string value = lua_debug_value(L, -1);
			Msg("* [STORY_DUMP][LUA][story_objects] key=%s value=%s", key.c_str(), value.c_str());
			++top_level_count;
			lua_pop(L, 1);
		}
		Msg("* [STORY_DUMP][LUA] story_objects top_level_count=%u", top_level_count);
	}
	...
	if (lua_istable(L, -1))
	{
		...
		while (lua_next(L, -2) != 0)
		{
			xr_string key = lua_debug_value(L, -2);
			xr_string value = lua_debug_value(L, -1);
			Msg("* [STORY_DUMP][LUA][object_id_by_story_id] key=%s value=%s", key.c_str(), value.c_str());
			++count;
			lua_pop(L, 1);
		}
		Msg("* [STORY_DUMP][LUA] object_id_by_story_id count=%u", count);
	}
	else
	{
		Msg("! [STORY_DUMP] story_objects.object_id_by_story_id is not table");
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "story_id_by_object_id");
	if (lua_istable(L, -1))
	{
		u32 count = 0;
		lua_pushnil(L);
		while (lua_next(L, -2) != 0)
		{
			xr_string key = lua_debug_value(L, -2);
			xr_string value = lua_debug_value(L, -1);
			Msg("* [STORY_DUMP][LUA][story_id_by_object_id] key=%s value=%s", key.c_str(), value.c_str());
			++count;
			lua_pop(L, 1);
		}
		Msg("* [STORY_DUMP][LUA] story_id_by_object_id count=%u", count);
	}
	else
	{
		Msg("! [STORY_DUMP] story_objects.story_id_by_object_id is not table");
	}
	lua_pop(L, 1);
...
u16 map_has_object_spot(u16 id, LPCSTR spot_type)
{
	...
	const u16 has = Level().MapManager().HasMapLocation(spot_type, id);
	if (is_level_changer_spot(spot_type))
	{
...
{
	if (g_dedicated_server) return &Actor()->GetPick();
