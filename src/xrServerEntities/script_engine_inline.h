...
template<typename F>
IC void CScriptEngine::InitializeFunctor(F*& func, const char* name, bool load_fl)
{
	func = new F();
	if (!functor(name, *func, load_fl))
	{
		delete func;
		func = nullptr;
	}
...
IC bool CScriptEngine::functor(LPCSTR function_to_call, luabind::functor<_result_type>& lua_function, bool load_fl)
{
	PROF_EVENT("CScriptEngine::functor");
	luabind::object object;
	if (!function_object(function_to_call, object, LUA_TFUNCTION, load_fl))
		return (false);
