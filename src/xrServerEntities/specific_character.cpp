...
void CSpecificCharacter::load_shared(LPCSTR)
{
	...
	timer.Start();
#endif
	const ITEM_DATA* item_data = id_to_index::GetById(m_OwnId, true);
	R_ASSERT3(item_data, "m_OwnId not found: ", *m_OwnId);
	if (!item_data)
		item_data = id_to_index::GetByIndex(0);
	...
	R_ASSERT3(!(data()->m_bNoRandom && data()->m_bDefaultForCommunity),
	          "cannot set 'no_random' and 'team_default' flags simultaneously, profile id", *item_data->id);
	...
	extern luabind::functor<luabind::object>* CSpecificCharacterDialogList;
	if (CSpecificCharacterDialogList)
	{
		...
		luabind::object output = (*CSpecificCharacterDialogList)(character_name, table);
	...
	extern luabind::functor<luabind::object>* CSpecificCharacterInit;
	if (CSpecificCharacterInit)
	{
		luabind::object table = luabind::newtable(ai().script_engine().lua());
		table["name"] = Name();
		table["bio"] = Bio().c_str();
		...
		luabind::object output = (*CSpecificCharacterInit)(item_data->id.c_str(), table);
		if (output && output.type() == LUA_TTABLE)
		{
