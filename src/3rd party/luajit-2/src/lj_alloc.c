...
#define CHUNKS_FROM_SIZE(x) ((x + CHUNK_SIZE - 1ULL) / CHUNK_SIZE)
#define BITS_PER_WORD 32u

static void* g_heap;
static uint32_t* g_heapMap;
static size_t g_totalChunks;
static size_t g_firstFreeChunk;

// Макросы для работы с битовой картой
#define BIT_MASK(pos) (1u << (pos % BITS_PER_WORD))
#define WORD_INDEX(pos) (pos / BITS_PER_WORD)

// Проверка свободен ли чанк
static LJ_AINLINE short is_chunk_free(size_t chunk_index)
{
    if (chunk_index >= g_totalChunks) return 0;
    return (g_heapMap[WORD_INDEX(chunk_index)] & BIT_MASK(chunk_index)) == 0;
}

// Установка состояния чанка
static LJ_AINLINE void set_chunk_state(size_t chunk_index, short allocated)
{
    if (chunk_index >= g_totalChunks) return;

    size_t word_idx = WORD_INDEX(chunk_index);
    uint32_t bit_mask = BIT_MASK(chunk_index);

    if (allocated) {
        g_heapMap[word_idx] |= bit_mask;
    }
    else {
        g_heapMap[word_idx] &= ~bit_mask;
    }
...
// Поиск последовательности свободных чанков
static LJ_AINLINE size_t find_free_chunks(size_t chunks_needed)
{
    if (chunks_needed == 0ULL || chunks_needed > g_totalChunks)
        return g_totalChunks;

    size_t consecutive_free = 0ULL;
    size_t current_start = g_firstFreeChunk;

    for (size_t i = g_firstFreeChunk; i < g_totalChunks; i++) {
        if (is_chunk_free(i)) {
            if (consecutive_free == 0ULL) {
                current_start = i; // Запоминаем начало последовательности
            }
            consecutive_free++;
            if (consecutive_free >= chunks_needed) {
                return current_start;
            }
        }
        else {
            consecutive_free = 0ULL;
        }
    }
...
static LJ_AINLINE void* XR_MMAP(size_t size)
{
    size_t chunks = CHUNKS_FROM_SIZE(size);
    if (chunks <= 0ULL) return MFAIL;

    size_t free_index = find_free_chunks(chunks);
    if (free_index == g_totalChunks) return MFAIL;

    // Помечаем чанки как занятые
    for (size_t i = 0ULL; i < chunks; i++) {
        set_chunk_state(free_index + i, 1);
    }

    // Обновляем первый свободный чанк
    if (free_index == g_firstFreeChunk) {
        g_firstFreeChunk = free_index + chunks;
        while (g_firstFreeChunk < g_totalChunks && !is_chunk_free(g_firstFreeChunk)) {
            g_firstFreeChunk++;
        }
    }

    return (char*)g_heap + free_index * CHUNK_SIZE;
}

static LJ_AINLINE void XR_DESTROY(void* ptr, size_t size)
{
    if (!ptr) return;

    size_t start_chunk = ((char*)ptr - (char*)g_heap) / CHUNK_SIZE;
    if (start_chunk >= g_totalChunks) return;

    size_t chunks = CHUNKS_FROM_SIZE(size);

    // Помечаем чанки как свободные
    for (size_t i = 0ULL; i < chunks; i++) {
        set_chunk_state(start_chunk + i, 0);
    }

    // Обновляем первый свободный чанк
    if (start_chunk < g_firstFreeChunk) {
        g_firstFreeChunk = start_chunk;
    }
}

LUALIB_API size_t luaL_create_allocated_block(void)
{
    ntavm = (PNTAVM)GetProcAddress(GetModuleHandleA("ntdll.dll"),
        "NtAllocateVirtualMemory");
    if (!ntavm)
    {
        MessageBoxAPtr pMessageBoxA = (MessageBoxAPtr)GetProcAddress(
            GetModuleHandleA("user32.dll"), "MessageBoxA");
        pMessageBoxA(NULL, "NtAllocateVirtualMemory not loaded",
            "LuaJIT Pool allocation error", MB_ICONERROR | MB_OK);
        exit(EXIT_FAILURE);
    }

    g_heap = NULL;
    size_t multiplier = 16ULL;
    size_t chunks_count = 2048ULL;
    size_t size = 0ULL;
    long st = 0xC0000225L;

    while (st != 0x00000000L)
    {
        size = CHUNK_SIZE * (chunks_count * multiplier);
        st = ntavm(INVALID_HANDLE_VALUE, &g_heap, NTAVM_ZEROBITS, &size,
        ...
        if (multiplier <= 0ULL)
        {
            MessageBoxAPtr pMessageBoxA = (MessageBoxAPtr)GetProcAddress(
                GetModuleHandleA("user32.dll"), "MessageBoxA");
            pMessageBoxA(NULL, "Failed to allocate memory at lower addresses",
                "LuaJIT Pool allocation error", MB_ICONERROR | MB_OK);
            exit(EXIT_FAILURE);
        }
    }

    // Вычисляем параметры битовой карты
    g_totalChunks = chunks_count * multiplier;
    size_t g_heapMapWords = (g_totalChunks + BITS_PER_WORD - 1u) / BITS_PER_WORD;

    // Выделяем память для битовой карты
    g_heapMap = (uint32_t*)calloc(g_heapMapWords, sizeof(uint32_t));
    if (!g_heapMap)
    {
        MessageBoxAPtr pMessageBoxA = (MessageBoxAPtr)GetProcAddress(
            GetModuleHandleA("user32.dll"), "MessageBoxA");
        pMessageBoxA(NULL, "Failed to allocate g_heapMap",
            "LuaJIT Pool allocation error", MB_ICONERROR | MB_OK);
        exit(EXIT_FAILURE);
    }

    g_firstFreeChunk = 0ULL;

    return size / 1024ULL / 1024ULL;
}

// Функция для очистки
LUALIB_API void luaL_destroy_allocated_block(void)
{
    if (g_heap) {
        size_t size = g_totalChunks * CHUNK_SIZE;
        ntavm(INVALID_HANDLE_VALUE, &g_heap, NTAVM_ZEROBITS, &size,
