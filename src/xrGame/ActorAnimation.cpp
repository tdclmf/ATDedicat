...
void SVehicleAnimCollection::Create(IKinematicsAnimated* V, u16 num)
{
	...
	idles[0] = V->ID_Cycle_Safe("steering_legs_idle");
	idles[1] = V->ID_Cycle_Safe("steering_torso_idle");
}
...
void CActor::steer_Vehicle(float angle)
{
	...
	if (angle == 0.f)
	{
		Visual()->dcast_PKinematicsAnimated()->PlayCycle(anims.idles[0]);
		Visual()->dcast_PKinematicsAnimated()->PlayCycle(anims.idles[1]);
	}
	else if (angle > 0.f)
		Visual()->dcast_PKinematicsAnimated()->PlayCycle(anims.steer_right);
	else
		Visual()->dcast_PKinematicsAnimated()->PlayCycle(anims.steer_left);
}
...
void CActor::g_SetAnimation(u32 mstate_rl)
{
	...
	if (this == Level().CurrentViewEntity() && !g_dedicated_server)
	{
		if (mstate_rl & mcAnyMove)
		{
			...
			else if ((mstate_rl & mcAccel) != (mstate_old & mcAccel) && !IsZoomAimingMode())
			{
