		...
		case ePathStateBuildLevelPath:
			{
				...
				//R_ASSERT(ai().game_graph().valid_vertex_id(object().ai_location().game_vertex_id()));
				//R_ASSERT(ai().level_graph().valid_vertex_id(object().ai_location().level_vertex_id()));

				u32 dest_level_vertex_id = ai().game_graph().vertex(
					game_path().intermediate_vertex_id()
				)->level_vertex_id();
				//R_ASSERT(ai().level_graph().valid_vertex_id(dest_level_vertex_id));
