...
void CEntityAlive::OnChangeVisual(bool destroy)
{
...
{
	if(getDestroy() || !Visual() || !Visual()->dcast_PKinematics() || bone_id == BI_NONE || bone_id >= Visual()->dcast_PKinematics()->LL_BoneCount())
		return inherited::get_last_local_point_on_mesh(last_point, bone_id);

	return Fvector(Visual()->dcast_PKinematics()->LL_GetTransform(bone_id).c).add(XFORM().c);
}
