...
ICF static BOOL GetPickDistSimple_Callback(collide::rq_result& result, LPVOID params)
{
	collide::rq_result* RQ = (collide::rq_result*)params;
...
static ICF collide::rq_result GetPickResult(Fvector pos, Fvector dir, float range, CObject* ignore, bool simple = false)
{
	collide::rq_result		RQ; RQ.set(NULL, range, -1);
	collide::rq_results		RQR;
	collide::ray_defs	RD(pos, dir, RQ.range, CDB::OPT_FULL_TEST, collide::rqtBoth);
	Level().ObjectSpace.RayQuery(RQR, RD, simple ? GetPickDistSimple_Callback : GetPickDist_Callback, &RQ, NULL, ignore);
	return RQ;
}
...
void CCameraLook::UpdateDistance(Fvector& point)
{
	...
	float final_dist = GetPickResult(point, vDir, dist + covariance, parent).range;

	float d = psCamSlideInert * prev_d + (1.f - psCamSlideInert) * (final_dist - covariance);
	prev_d = d;
