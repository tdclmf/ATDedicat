...
void CTorch::LoadLightParams()
{
	...
	has_volumetric_quiality = READ_IF_EXISTS(pSettings, r_float, light_definition.c_str(), "volumetric_quality", 1.f);
	light_render->set_volumetric(!!READ_IF_EXISTS(pSettings, r_bool, light_definition.c_str(), "volumetric", 0));
	light_render->set_volumetric_quality(has_volumetric_quiality);
	light_render->set_volumetric_intensity(READ_IF_EXISTS(pSettings, r_float, light_definition.c_str(), "volumetric_intensity",1.f));
	light_render->set_volumetric_distance(READ_IF_EXISTS(pSettings, r_float, light_definition.c_str(), "volumetric_distance",1.f));
...
extern BOOL enable_actortorch_volumetric;
void CTorch::UpdateCL()
{
	...
	if (H_Parent())
	{
		...
		if (CActor* actor = H_Parent()->cast_actor())
		{
			...
			if(!enable_actortorch_volumetric)
			{
				if (actor->active_cam() == eacFirstEye)
				{
					light_render->set_volumetric_quality(0.0f);
				}
				else
					light_render->set_volumetric_quality(has_volumetric_quiality);
			}
