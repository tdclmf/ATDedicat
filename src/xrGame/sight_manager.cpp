...
void CSightManager::Exec_Look(float time_delta)
{
	...
	if ( g_ai_dbg_sight )
		...
		static auto fpu_validator = [](float& v)
		{
			if (_isnanf(v))
				v = 0.f;
		};

		fpu_validator(body.current.yaw);
		fpu_validator(body.current.pitch);
		fpu_validator(body.current.roll);

		fpu_validator(body.target.yaw);
		fpu_validator(body.target.pitch);
		fpu_validator(body.target.roll);

		fpu_validator(head.current.yaw);
		fpu_validator(head.current.pitch);
		fpu_validator(head.current.roll);

		fpu_validator(head.target.yaw);
		fpu_validator(head.target.pitch);
		fpu_validator(head.target.roll);
