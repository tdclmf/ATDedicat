...
{
	if (m_holder || g_dedicated_server) return;
	...
	if (cam_active != eacFreeLook && (mstate_real & mcClimb))
	{
	...
	if (cam_active == eacFirstEye)
	{
		if (firstPersonDeath && !g_Alive())
		{
			...
			Fmatrix BoneMatrix, BoneMatrixwrld;
			Fobb BoneOBB = k->LL_GetBox(m_head);
			BoneOBB.xform_get(BoneMatrix);
			BoneMatrix.mul(XFORM(), Fmatrix().mul(headBone.mTransform, BoneMatrix));
			BoneMatrixwrld.mul(XFORM(), headBone.mTransform);
			C->vDirection.set(BoneMatrixwrld.k);
			C->vNormal.set(BoneMatrixwrld.i);
			C->vPosition.set(BoneMatrix.c);
			collide_camera(*C, _viewport_near, this);
		}
	}
