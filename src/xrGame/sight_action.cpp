	...
	if (count > 1)
	{
		...
		if (Device.dwTimeGlobal - previous_position.dwTime < 300)
		{
			if (current_position.dwTime > previous_position.dwTime)
			{
				...
				offset.y = 0.f;
				float divider = float(current_position.dwTime - previous_position.dwTime);
				divider = divider > 0.f ? divider / 1000.f : 0.f;

				Fvector velocity = divider > 0.f ? Fvector(offset).div(divider) : offset;
