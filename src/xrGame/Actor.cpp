...
void CActor::Hit(SHit* pHDS)
{
	...
	else
	{
		if (g_Alive())
		{
			...
			extern luabind::functor<bool>* CActor__BeforeHitCallback;
			if (CActor__BeforeHitCallback)
			{
				if (!(*CActor__BeforeHitCallback)(this->lua_game_object(), &tLuaHit, HDS.boneID))
					return;
			}
...
void CActor::g_Physics(Fvector& _accel, float jump, float dt)
{
	...
	if (g_Alive())
	{
		...
		if (psActorFlags.test(AF_NO_CLIP))
			character_physics_support()->movement()->SetVelocity(zero_vel);
		else
...
float NET_Jump = 0;
extern BOOL precise_catspaw_dotmarks;
void CActor::UpdateCL()
{
	...
	cam_Update(Device.fTimeDelta, currentFOV());
	...
	if (IsFocused() && this == Level().CurrentEntity())
	...
	//Discord
	if (psDeviceFlags2.test(rsDiscord) && this == Level().CurrentEntity())
	{
		//God
		...
		{
			extern luabind::functor<bool>* IsHardcoreMode;
			extern luabind::functor<int>* get_lives_left;
			extern luabind::functor<bool>* IsStoryMode;
			extern luabind::functor<bool>* IsWarfare;
			extern luabind::functor<bool>* IsAzazelMode;
			extern luabind::functor<bool>* IsSurvivalMode;
			extern luabind::functor<int>* get_possessed_lives;

			//Update Iron Man state and lives
			if (IsHardcoreMode && (*IsHardcoreMode)())
	...
	if (!psHUD_Flags.test(HUD_DRAW) && CurrentGameUI())
		CurrentGameUI()->UIMainIngameWnd->get_hud_states()->UpdateZones();

	if (!getVisible() && this != Actor())
		setVisible(TRUE);
}
...
void CActor::RPC_UpdateFaction()
{
	...
	extern luabind::functor<LPCSTR>* get_actor_true_community;
	if (get_actor_true_community)
	{
		LPCSTR faction_name = (*get_actor_true_community)();
...
{
	//Rank		
	extern luabind::functor<LPCSTR>* get_player_rank_name;
	if (get_player_rank_name)
	{
		LPCSTR rank = (*get_player_rank_name)();
		char rank_name[100];
...
{
	//Reputation		
	extern luabind::functor<int>* get_player_reputation;
	if (get_player_reputation)
	{
		extern luabind::functor<LPCSTR>* get_reputation_name;
		if (get_reputation_name)
		{
			int reputation = (*get_player_reputation)();
			if (LPCSTR reputation_name = (*get_reputation_name)(reputation))
...
IC void ForceSetPosition(Fmatrix& M, CGameObject* object)
{
...
void CActor::on_requested_spawn(CObject *object)
{
	...
	ForceSetPosition(XFORM(), object->cast_game_object());

	use_HolderEx(oHolder, true);

	if(m_holder && !m_holder->OwnerActor())
		m_holder->attach_Actor(this);
...
{
	VERIFY(_valid(XFORM()));
	if (this != Actor()) // draw character
	{
		inherited::renderable_Render();
		CInventoryOwner::renderable_Render();
		return;
	}
	if (cam_active == eacFirstEye && !m_holder)
	{
...
void CActor::ForceTransform(const Fmatrix& m)
{
	...
	character_physics_support()->movement()->BlockDamageSet(u64(2.f / fixed_step));
}
...
{
	extern luabind::functor<luabind::object>* CActor__HitArtefactsOnBelt;
	if (CActor__HitArtefactsOnBelt)
	{
		luabind::object table = luabind::newtable(ai().script_engine().lua());
		table["override"] = false;
		table["hit_power"] = hit_power;

		luabind::object output = (*CActor__HitArtefactsOnBelt)(table, hit_power, hit_type);
...
void CActor::OnDifficultyChanged()
{
	...
	m_hit_probability = READ_IF_EXISTS(pSettings, r_float, cNameSect(), tmp, 
		pSettings->r_float(*cNameSect(), "hit_probability_master"));
...
CObject* CActor::ParentHolder()
{
	if (m_holder)
	{
		if (CCar* pCar = smart_cast<CCar*>(m_holder))
			return pCar;

		if (CWeaponStatMgun* pSmg = smart_cast<CWeaponStatMgun*>(m_holder))
			return pSmg;

		if (CHolderEntityObject* pHeo = smart_cast<CHolderEntityObject*>(m_holder))
			return pHeo;
	}
