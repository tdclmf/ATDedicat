...
void CActorCondition::UpdateCondition()
{
	...
	if (Actor() == &object() && psActorFlags.test(AF_GODMODE_RT))
	{
		UpdateSatiety();
		UpdateBoosters();
		...
		CEffectorCam* ce = object().Cameras().GetCamEffector((ECamEffectorType)effAlcohol);
		if (ce)
	...
	if (GetHealth() < 0.05f && m_death_effector == NULL && pSettings->section_exist("actor_death_effector"))
...
CActorDeathEffector::CActorDeathEffector(CActorCondition* parent, LPCSTR sect) // -((
	: m_pParent(parent)
{
	if (Actor() == &parent->object()) {
		parent->object().SetWeaponHideState(INV_STATE_BLOCK_ALL, true);
		hide_indicators();
		...
		SBaseEffector* pe = parent->object().Cameras().GetPPEffector((EEffectorPPType)effActorDeath);
		if (pe)
