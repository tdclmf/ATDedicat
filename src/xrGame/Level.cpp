...
void CLevel::UI_AND_PHCommanders()
{
	PROF_EVENT("UI_AND_PHCommanders");
	if (g_hud && !g_dedicated_server)
		g_hud->OnFrame();

	if (m_map_manager)
		m_map_manager->Update();
	if (m_game_task_manager)
		m_game_task_manager->UpdateTasks();
...
void CLevel::UpdateBulletManager()
{
	BulletManager().UpdateWorkload();
}

void CLevel::SoundEvent_Dispatch()
{
	PROF_EVENT("IGame_Level::SoundEvent_Dispatch");

	inherited::SoundEvent_Dispatch();
}
BOOL psLUA_GCOLD = FALSE;
BOOL psLUA_GCLEAKPRINT = FALSE;
int psLUA_GCSTEP = 400;
void CLevel::script_gc_start()
{
	if (psLUA_GCOLD) return;
	lua_gc(ai().script_engine().lua(), LUA_GCRESTART, 0);
}

void CLevel::script_gc_stop()
{
	if (psLUA_GCOLD) return;
...
void CLevel::script_gc()
{
	...
	if(psLUA_GCLEAKPRINT)
		Msg("Lua GC: trashcan: kb[%d]", lua_gc(lua_state, LUA_GCCOUNT, 0));
...
extern xrCriticalSection render_hud_cs;
void CLevel::Draw3dPDA()
{
...
{
	if (!m_debug_render_queue.size() || g_dedicated_server)
		return;
