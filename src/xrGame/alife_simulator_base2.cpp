...
{
	PROF_EVENT("CALifeSimulatorBase::register_object");
	...
	if (item && item->attached() && item->base())
	{
		if(CSE_ALifeDynamicObject* II = objects().object(item->base()->ID_Parent))
		{
...
void CALifeSimulatorBase::unregister_object(CSE_ALifeDynamicObject* object, bool alife_query)
{
	...
	if(CSE_ALifeInventoryItem* item = smart_cast<CSE_ALifeInventoryItem*>(object))
	{
		if (item && item->attached() && item->base())
