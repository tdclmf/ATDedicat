...
extern BOOL psLUA_GCOLD;
extern BOOL psLUA_GCLEAKPRINT;
...
class CCC_StlkWound : public IConsole_Command
{
public:
	CCC_StlkWound(LPCSTR N)
		: IConsole_Command(N) {
	};
	virtual void Execute(LPCSTR args)
	{
		...
		if (HasPick && RQ.O)
		{
			CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(RQ.O);
			if (stalker)
			{
				stalker->SetfHealth(0.1f);
...
class CCC_TComm : public IConsole_Command
{
public:
	CCC_TComm(LPCSTR N)
		: IConsole_Command(N) {
	...
	virtual void Execute(LPCSTR args)
	{
		...
		if (HasPick && RQ.O)
		{
			CAI_Stalker* pstalk = smart_cast<CAI_Stalker*>(RQ.O);
			if (pstalk)
			{
				CInventoryOwner* pstalkinv = smart_cast<CInventoryOwner*>(pstalk);
				if (pstalkinv)
				{
					CHARACTER_COMMUNITY	community;
					community.set(args);
					pstalkinv->SetCommunity(community.index());
				}
			}
		}
		else
		{
			if (Actor())
			{
				CHARACTER_COMMUNITY	community;
				community.set(args);
				Actor()->SetCommunity(community.index());
			}
		}
	}
	virtual void fill_tips(vecTips& tips, u32 mode)
...
class CCC_TCommRelat : public IConsole_Command
{
public:
	CCC_TCommRelat(LPCSTR N)
		: IConsole_Command(N) {
	};
	virtual void Execute(LPCSTR args)
	{
		...
		int count = 0;
		char name_comm[128] = {};
		auto sc = sscanf_s(args, "%s %d", name_comm, (unsigned)sizeof(name_comm), &count);
		if (sc > 2) {
		...
		{
			CHARACTER_COMMUNITY	community;
			community.set(name_comm);
			RELATION_REGISTRY().SetCommunityRelation(Actor()->Community(), community.index(), count);

			//CommunityGoodwill
			RELATION_DATA& relation_data = RELATION_REGISTRY().relation_registry().registry().objects(Actor()->ID());
			relation_data.communities[community.index()].SetGoodwill(count);
			//RELATION_REGISTRY().SetCommunityGoodwill( community.index(), Actor()->ID(), count);
		}
	}
	virtual void fill_tips(vecTips& tips, u32 mode)
	{
		for (CHARACTER_COMMUNITY_INDEX i = 0; i < CHARACTER_COMMUNITY().GetMaxIndex() + 1; ++i)
			tips.push_back(CHARACTER_COMMUNITY().GetByIndex(i)->id.c_str());
	}
	virtual void Info(TInfo& I) { strcpy(I, "spawn_entities"); }
};
#include "map_manager.h"
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		{
			Msg("! Section [%s] isn`t exist...", nameSection);
			return;
		}
		if (!pSettings->line_exist(nameSection, "class"))
		{
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		if (!pSettings->line_exist(nameSection, "class") || !pSettings->line_exist(nameSection, "inv_weight") || !pSettings->line_exist(nameSection, "visual"))
		{
			Msg("!Failed to load section!");
			return;
		}
...
public:
	...
	virtual void Execute(LPCSTR weather_name) {
		if (!g_pGamePersistent && OnClient())
			return;

		if (weather_name && weather_name[0]) {
			g_pGamePersistent->Environment().SetWeather(weather_name, OnServer());
		}
	}
...
void CCC_RegisterCommands()
{
	...
	CMD1(CCC_Spawn_to_inventory, "g_spawn_inv");
	CMD1(CCC_SpawnSquad, "g_spawn_squad");
	CMD1(CCC_TComm, "g_tscommunity");
	CMD1(CCC_TCommRelat, "g_tscommunity_relat");
	...
	CMD4(CCC_Integer, "precise_catspaw_dotmarks", &precise_catspaw_dotmarks, 0, 1);
	CMD4(CCC_Integer, "enable_actortorch_volumetric", &enable_actortorch_volumetric, 0, 1);
	...
	CMD4(CCC_Integer, "lua_gcstep", &psLUA_GCSTEP, 0, 4000);
	CMD4(CCC_Integer, "lua_gc_old", &psLUA_GCOLD, 0, 1);
	CMD4(CCC_Integer, "lua_gc_leak_print", &psLUA_GCLEAKPRINT, 0, 1);
#ifdef DEBUG
