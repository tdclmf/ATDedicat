...
void CUILogsWnd::ReLoadNews()
{
	...
	m_need_reload = false;
	xrCriticalSection::raii guard(&m_list->Pad()->childs_cs);
	if (!m_list->Pad()->GetChildWndList().empty())
	{
		m_items_cache.insert(m_items_cache.end(), m_list->Pad()->GetChildWndList().begin(), m_list->Pad()->GetChildWndList().end());
		m_list->Pad()->GetChildWndList().clear();
