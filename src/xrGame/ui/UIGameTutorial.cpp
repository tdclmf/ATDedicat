...
void CUISequencer::IR_OnKeyboardPress(int dik)
{
	...
	bool b = true;
	if (!m_sequencer_items.empty()) b &= m_sequencer_items.front()->AllowKey(dik);
	...
	CUIGameCustom* MUI = CurrentGameUI();
	if (binded && MUI)
	{
		if (MUI->GetActorMenu().IsShown())
		{
			MUI->HideActorMenu();
			return;
		}
		if (MUI->TopInputReceiver() && MUI->TopInputReceiver()->IsShown())
		{
			MUI->TopInputReceiver()->OnKeyboardAction(dik, WINDOW_KEY_PRESSED);
			return;
		}
