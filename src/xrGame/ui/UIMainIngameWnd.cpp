...
void CUIMainIngameWnd::Update()
{
	...
	else if (gid == eGameIDCaptureTheArtefact)
	{
		...
		if (!cta_game) return;
		u16 ownid = cta_game->GetGreenArtefactOwnerID();
		if (pActor->ID() == ownid || pActor->ID() == ownid)
		{
...
void CUIMainIngameWnd::UpdateBoosterIndicators(xr_map<EBoostParams, SBooster>& influences)
{
	m_ind_boost_psy->Show(false);
	m_ind_boost_radia->Show(false);
	m_ind_boost_chem->Show(false);
