                                                     ...
                                                     bool full_mode)
{
	...
	{
	case etpTimeToHours:
		cur_time.printf("%02i", hours);
		break;
	case etpTimeToMinutes:
		if (full_mode || hours > 0)
		{
			cur_time.printf("%02i%c%02i", hours, timeSeparator, mins);
			break;
		}
		cur_time.printf("0%c%02i", timeSeparator, mins);
		break;
	case etpTimeToSeconds:
		if (full_mode || hours > 0)
		{
			cur_time.printf("%02i%c%02i%c%02i", hours, timeSeparator, mins, timeSeparator, secs);
			break;
		}
		if (mins > 0)
		{
			cur_time.printf("%02i%c%02i", mins, timeSeparator, secs);
			break;
		}
		cur_time.printf("0%c%02i", timeSeparator, secs);
		break;
	case etpTimeToMilisecs:
		cur_time.printf("%02i%c%02i%c%02i%c%02i", hours, timeSeparator, mins, timeSeparator, secs, timeSeparator,
		           milisecs);
		break;
	case etpTimeToSecondsAndDay:
		{
			int total_day = (int)(time / (1000 * 60 * 60 * 24));
			cur_time.printf("%dd %02i%c%02i%c%02i", total_day, hours, timeSeparator, mins,
			           timeSeparator, secs);
			break;
		}
	default:
