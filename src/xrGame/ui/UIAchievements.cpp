...
{
	xrCriticalSection::raii guard(&m_parent->Pad()->childs_cs);
	WINDOW_LIST::const_iterator it = std::find(m_parent->Pad()->GetChildWndList().begin(), m_parent->Pad()->GetChildWndList().end(), this);
	return it != m_parent->Pad()->GetChildWndList().end();
}
