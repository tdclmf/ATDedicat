...
{
	return CurrentGameUI() ? &CurrentGameUI()->GetActorMenu() : 0;
}

CUIPdaWnd* GetPDAMenu()
{
	return CurrentGameUI() ? &CurrentGameUI()->GetPdaMenu() : 0;
}

CUIMainIngameWnd* GetMainGameMenu()
{
	return CurrentGameUI() ? CurrentGameUI()->UIMainIngameWnd : 0;
}

CUIMessagesWindow* GetMessagesMenu()
{
	return CurrentGameUI() ? CurrentGameUI()->m_pMessagesWnd : 0;
}

u8 GrabMenuMode()
{
	return CurrentGameUI()->GetActorMenu().GetMenuMode();
}
