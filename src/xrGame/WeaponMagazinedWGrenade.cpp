...
{
	CSE_ALifeItemWeapon* weapon = smart_cast<CSE_ALifeItemWeapon*>(DC);
	VERIFY(weapon);
...
bool CWeaponMagazinedWGrenade::GetBriefInfo(II_BriefInfo& info)
{
	...
	else
	{
		...
		if (m_ammoType == 0)
			info.fmj_ammo.printf("%d", m_bGrenadeMode ? GetAmmoCount2(ammo_type) : GetAmmoCount(ammo_type));
		else if (m_ammoType == 1)
			info.ap_ammo.printf("%d", m_bGrenadeMode ? GetAmmoCount2(ammo_type) : GetAmmoCount(ammo_type));
		else
			info.third_ammo.printf("%d", m_bGrenadeMode ? GetAmmoCount2(ammo_type) : GetAmmoCount(ammo_type));
		...
		if (at_size >= 1)
			info.fmj_ammo.printf("%d", m_bGrenadeMode ? GetAmmoCount2(0) : GetAmmoCount(0));
		if (at_size >= 2)
			info.ap_ammo.printf("%d", m_bGrenadeMode ? GetAmmoCount2(1) : GetAmmoCount(1));
		if (at_size >= 3)
			info.third_ammo.printf("%d", m_bGrenadeMode ? GetAmmoCount2(2) : GetAmmoCount(2));
	}
