...
{
	Msg("CGameObject::net_Spawn -- Destroying [%d][%s] client object", ID(), *cName());

	VERIFY(m_spawned);

	extern luabind::functor<void>* CGameObject_NetDestroy;
	if (CGameObject_NetDestroy)
		(*CGameObject_NetDestroy)(this->lua_game_object());
...
extern void __stdcall AttachmentCallback(IKinematics* tpKinematics);
...
#pragma warning(pop)
	...
	spawn_supplies();
	if (cast_attachment_owner())
		add_visual_callback(AttachmentCallback);
#ifdef DEBUG
	if(ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject)&&stricmp(PH_DBG_ObjectTrackName(),*cName())==0)
	{
...
#else
	extern luabind::functor<void>* CGameObject_NetSpawn;
	if (CGameObject_NetSpawn)
	{
		(*CGameObject_NetSpawn)(this->lua_game_object());
	}
...
void CGameObject::setup_parent_ai_locations(bool assign_position)
{
	...
	bool pos_update = use_parent_ai_locations();
	if (H_Parent())
	{
		if (getVisible())
			pos_update = false;
		...
		{
			if (item->m_pInventory && item == item->m_pInventory->ActiveItem())
				pos_update = false;
		}

		if (CAttachableItem* Aitem = cast_attachable_item())
		{
			if (Aitem->enabled())
				pos_update = false;
		}
		if (cast_weapon() && cast_weapon()->m_pInventory && (cast_weapon() == cast_weapon()->m_pInventory->ItemFromSlot(INV_SLOT_2) || cast_weapon() == cast_weapon()->m_pInventory->ItemFromSlot(INV_SLOT_3) || cast_weapon() == cast_weapon()->m_pInventory->ItemFromSlot(GRENADE_SLOT)))
			pos_update = false;
	}
	if (assign_position && pos_update)
...
void CGameObject::OnChangeVisual(bool destroy)
{
	inherited::OnChangeVisual(destroy);
	if (m_anim_mov_ctrl)
		destroy_anim_mov_ctrl();
}
