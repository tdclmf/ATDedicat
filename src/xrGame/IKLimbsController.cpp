...
{
	PROF_EVENT("CIKLimbsController::Calculate");
...
void _stdcall CIKLimbsController::IKVisualCallback(IKinematics* K)
{
	...
	if( ph_dbg_draw_mask1.test( phDbgIKOff ) )
		return;
#endif
	if (!ik_available) return;
	CGameObject* O = ((CGameObject*)K->GetUpdateCallbackParam());
	CPhysicsShellHolder* Sh = smart_cast<CPhysicsShellHolder*>(O);
	VERIFY(Sh);
	CIKLimbsController* ik = Sh->character_ik_controller();
	VERIFY(ik);
	if (O->cast_actor() && O->cast_actor()->active_cam() == eacFirstEye)
		return;

	{
		PROF_EVENT("CIKLimbsController::Update");
		ik->_pose_extrapolation.update(ik->m_object->XFORM());
		xr_vector<CIKLimb>::iterator i = ik->_bone_chains.begin(), e = ik->_bone_chains.end();
		for (; e != i; ++i)
			ik->LimbUpdate(*i);
	}
