...
{
	static xr_vector<PIItem> next_grenades;
	PIItem next_grenade = NULL;

	for (PIItem item : m_ruck)
	{
		if (CGrenade* pGrenade = item&&item->cast_game_object() ? item->cast_game_object()->cast_grenade() : NULL)
		{
			if(CGrenade* pGrenadeActive = ActiveItem()&&ActiveItem()->cast_game_object() ? ActiveItem()->cast_game_object()->cast_grenade() : NULL)
			{
				if (pGrenadeActive->cNameSect().equal(pGrenade->cNameSect()))
					continue;
			}
			next_grenades.push_back(pGrenade->cast_inventory_item());
		}
	}

	std::sort(next_grenades.begin(), next_grenades.end(), [](PIItem a, PIItem b) {
		return xr_strcmp(a->object().cNameSect(), b->object().cNameSect()) < 0;
		});

	if(!next_grenades.empty())
		next_grenade = next_grenades.front();
