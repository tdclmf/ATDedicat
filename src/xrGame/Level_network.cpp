...
void CLevel::Send(NET_Packet& P, u32 dwFlags, u32 dwTimeout)
{
	...
	if (Server)
		Server->OnMessage(P, Server->OpenSocket);
...
bool CLevel::Connect2Server(const char* options)
{
	...
	//---------------------------------------------------------------------------
	if (!m_bConnectResultReceived && !Connect(options)) return FALSE;
	u64 EndTime = GetTickCount64() + ConnectionTimeOut;
	while (!m_bConnectResultReceived)
	{
		ClientReceive();
		Sleep(5);
...
{
	PROF_EVENT("CLevel::ClearAllObjects");
