                        ...
                        u32 id)
{
	...
	shared_str name;
	name.printf("%s%s%i", *sound.m_sound_player_prefix, *sound.m_sound_prefix, (random_id + 1));
	//Msg("Sound name \"%s\"", name);
	extern luabind::functor<void>* CAI_Stalker_OnPhraseCallback;
	if (CAI_Stalker_OnPhraseCallback)
	{
		if (CGameObject* g_obj = m_object ? m_object->cast_game_object():NULL)
			(*CAI_Stalker_OnPhraseCallback)(name.c_str(), g_obj->lua_game_object());
	};
...
IC Fvector CSoundPlayer::compute_sound_point(const CSoundSingle& sound)
{
	...
	l_tMatrix.mul_43(m_object->XFORM(),
	                 PKinematics(m_object->Visual())->LL_GetBoneInstance(sound.m_bone_id).mTransform);
	return (l_tMatrix.c);
}
