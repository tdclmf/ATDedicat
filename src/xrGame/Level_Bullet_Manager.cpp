...
static IC void populateBulletTable (
...
CBulletManager::~CBulletManager()
{
	m_Bullets.clear();
	m_WhineSounds.clear();
	m_ExplodeSounds.clear();
	m_Events.clear();
}

void CBulletManager::Load()
{
	char const* bullet_manager_sect = "bullet_manager"; // werasik2aa bullet manager mp was
	...
	if(LPCSTR explode_sounds = pSettings->line_exist(bullet_manager_sect, "explode_sounds") ? pSettings->r_string(bullet_manager_sect, "explode_sounds") : NULL)
	{
		int cnt = _GetItemCount(explode_sounds);
		xr_string tmp;
		for (int k = 0; k < cnt; ++k)
		{
			m_ExplodeSounds.push_back(ref_sound());
			m_ExplodeSounds.back().create(_GetItem(explode_sounds, k, tmp), st_Effect, sg_SourceType);
		}
	}
...
void CBulletManager::PlayExplodeSound(CObject* object, const Fvector& pos)
{
	if (m_ExplodeSounds.empty()) return;

	m_ExplodeSounds[Random.randI(0, m_ExplodeSounds.size())].play_at_pos(object, pos);
}
...
void CBulletManager::PlayWhineSound(SBullet* bullet, CObject* object, const Fvector& pos)
{
	...
	m_WhineSounds[Random.randI(0, m_WhineSounds.size())].play_no_feedback(object,0,0.f, &position);
}

void CBulletManager::Clear()
{
	{
		xrSRWLockGuard guard(&lock_bullets, false);
		m_Bullets.clear();
	}
	{
		xrSRWLockGuard guard(&lock_events, false);
		m_Events.clear();
	}
}
                               ...
                               int iShotNum)
{
	...
	extern luabind::functor<void>* CBulletOnInit;
	if (CBulletOnInit)
	{
...
void CBulletManager::UpdateWorkload()
{
	...
	xrSRWLockGuard guard(&lock_bullets, true);
...
BOOL CBulletManager::firetrace_callback(collide::rq_result& result, LPVOID params)
{
	...
	update_bullet(bullet, data, gravity, bullet.air_resistance);
...
static bool try_update_bullet(SBullet& bullet, Fvector const& gravity, float const air_resistance, float const time)
{
	...
	extern luabind::functor<void>* CBulletOnUpdate;
	if (CBulletOnUpdate)
	{
		luabind::object table = luabind::newtable(ai().script_engine().lua());
		populateBulletTable(
...
bool CBulletManager::process_bullet(collide::rq_results& storage, SBullet& bullet, u32 const delta_time)
{
	...
	for (;;)
	{
		for (;;)
		{
			...
			float time = trajectory_select_pick_time(bullet, low, high, gravity, bullet.air_resistance);
			if (time == low)
				return (false);

			float safe_time = time;
			VERIFY2(safe_time <= high, make_string("safe_time[%f], high[%f]", safe_time, high));
			if (!trajectory_check_error(previous_position, storage, bullet, low, time, gravity, bullet.air_resistance))
			{
			...
			if (!try_update_bullet(bullet, gravity, bullet.air_resistance, time))
				return (false);
		...
		case EVENT_HIT:
			{
				...
				extern luabind::functor<void>* CBulletOnImpact;
				if (CBulletOnImpact)
				{
					luabind::object table = luabind::newtable(ai().script_engine().lua());
					populateBulletTable(
		...
		case EVENT_REMOVE:
			{
				...
				extern luabind::functor<void>* CBulletOnRemove;
				if (CBulletOnRemove)
				{
					luabind::object table = luabind::newtable(ai().script_engine().lua());
					populateBulletTable(
	...
	case EVENT_HIT:
		{
			...
			if (_dynamic)
			{
				...
				{
					CGameObject* pGO = smart_cast<CGameObject*>(R.O); // werasik2aa not sure was checks
					bullet->targetID = R.O->ID();
				}
			};
		}
