...
Fvector CScriptGameObject::Center(bool bHud)
{
	if (bHud)
	{
		if (CHudItem* itm = object().cast_inventory_item() ? object().cast_inventory_item()->cast_hud_item() : NULL)
		{
...
u16 CScriptGameObject::bone_id(LPCSTR bone_name, bool bHud)
{
	...
	if (bHud && !g_dedicated_server)
	{
		if (CHudItem* itm = object().cast_inventory_item() ? object().cast_inventory_item()->cast_hud_item() : NULL)
			k = itm->HudItemData() ? itm->HudItemData()->m_model : nullptr;
		else if (object().cast_actor() && Actor() && object().ID() == Actor()->ID())
			k = g_player_hud->m_model ? g_player_hud->m_model->dcast_PKinematics() : nullptr;
	} else {
		k = object().Visual() ? object().Visual()->dcast_PKinematics() : nullptr;
	}
...
Fmatrix CScriptGameObject::bone_transform(u16 bone_id, bool bHud)
{
	...
	if (bone_id == BI_NONE || bone_id >= k->LL_BoneCount()) {
	...
	Fmatrix matrix = k->LL_GetTransform(bone_id);
	matrix.mulA_43(*xform);
	return matrix;
}
...
Fvector CScriptGameObject::bone_position(u16 bone_id, bool bHud)
{
	...
	Fvector pos = k->LL_GetTransform(bone_id).c;
...
luabind::object CScriptGameObject::list_bones(bool bHud)
{
	...
	for (const auto& bone : *k->LL_Bones())
...
CGameObject& CScriptGameObject::object() const
{
	...
	__except (EXCEPTION_EXECUTE_HANDLER) { return (*m_game_object_dummy); }
