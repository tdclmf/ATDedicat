...
void CLevel::IR_OnMouseWheel(int direction)
{
    ...
    if (Actor()) {
        ...
        //Actor()->callback(GameObject::eMouseWheel)(direction);
    }
#endif
    /* avo: end */
...
#ifdef MOUSE_INPUT_CALLBACKS
    /* avo: script callback */
    if (Actor()) Actor()->callback(GameObject::eMouseMove)(dx, dy);
    /* avo: end */
...
void CLevel::IR_OnKeyboardPress(int key)
{
    ...
    if (!g_bDisableAllInput && Actor()) Actor()->callback(GameObject::eKeyPress)(key);
#endif
    /* avo: end */
    ...
    case DIK_BACK:
        if (OnServer()) DRender->NextSceneMode();
        return;
...
#ifdef INPUT_CALLBACKS
    /* avo: script callback */
    if (Actor()) Actor()->callback(GameObject::eKeyRelease)(key);
    /* avo: end */
#endif // INPUT_CALLBACKS
...
#ifdef INPUT_CALLBACKS
    /* avo: script callback */
    if (Actor()) Actor()->callback(GameObject::eKeyHold)(key);
    /* avo: end */
#endif // INPUT_CALLBACKS
