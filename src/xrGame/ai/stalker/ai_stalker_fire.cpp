...
float CAI_Stalker::GetWeaponAccuracy() const
{
	...
	extern luabind::functor<float>* CAI_Stalker__GetWeaponAccuracy;
	if (CAI_Stalker__GetWeaponAccuracy)
	{
		base = (*CAI_Stalker__GetWeaponAccuracy)(lua_game_object(), W ? W->lua_game_object() : nullptr, base, movement().body_state(), movement().movement_type());
	}
...
#pragma todo("Dima to Dima : forward-back bone impulse direction has been determined incorrectly!")
	...
	if (g_Alive() && (!m_hit_callback || m_hit_callback(&HDS)))
	{
		...
		extern luabind::functor<bool>* CAI_Stalker__BeforeHitCallback;
		if (CAI_Stalker__BeforeHitCallback)
		{
			if (!(*CAI_Stalker__BeforeHitCallback)(this->lua_game_object(), &tLuaHit, HDS.boneID))
				return;
		}
...
{
	extern luabind::functor<CScriptGameObject*>* Fupdate_best_weapon;
	if (Fupdate_best_weapon)
	{
		CGameObject* cur_itm = smart_cast<CGameObject*>(m_best_item_to_kill);
		CScriptGameObject* GO = (*Fupdate_best_weapon)(this->lua_game_object(), cur_itm ? cur_itm->lua_game_object() : NULL);
...
IC BOOL ray_query_callback(collide::rq_result& result, LPVOID params)
{
	...
	CEntityAlive* entity_alive = result.O&&result.O->cast_game_object() ? result.O->cast_game_object()->cast_entity_alive() : NULL;
	if (!entity_alive)
	{
		if (param->m_power > param->m_power_threshold)
			return (true);
...
{
	float result = 50.f;
	//critical!!! memory().enemy().selected() unsafed!!!
	if (memory().enemy().selected())
	{
		TRY
			return (
...
{
	if (m_pick_frame_id == Device.dwFrame || !inventory().ActiveItem())
		return;
