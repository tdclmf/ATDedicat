...
void CControlPathBuilderBase::find_node()
{
	...
	if (m_cover_info.use_covers && m_cover_approach)
	{
		m_cover_approach->setup(m_target_found.position(), m_cover_info.min_dist, m_cover_info.max_dist, m_cover_info.deviation);
		// нашли кавер?	
		if (const CCoverPoint* point = ai().cover_manager().best_cover(m_object->Position(), m_cover_info.radius, *m_cover_approach))
		{
