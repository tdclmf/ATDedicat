...
public:
	...
	u32 get_enemies_count() { xrCriticalSection::raii guard(&m_cs); return m_objects.size(); }

	const ENEMIES_MAP& get_memory() { xrCriticalSection::raii guard(&m_cs); return m_objects; }

	void clear() { xrCriticalSection::raii guard(&m_cs); m_objects.clear(); }
