...
BOOL CBulletManager::test_callback(const collide::ray_defs& rd, CObject* object, LPVOID params)
{
	...
	if (entity && entity->g_Alive() && (entity->ID() != bullet->parent_id))
	{
		...
		if ((NULL != cform) && (cftObject == cform->Type()))
		{
			...
			// в кого попали?
			if (actor && OnServer() || stalker) // not sure
			{
                                  ...
                                  u16 target_material, const Fvector& vNormal, bool ShowMark)
{
	...
	float dist_to_cam_sqr = Device.vCameraPosition.distance_to_sqr(vEnd);
	...
	//проиграть звук
	if ((pSound && ShowMark) || bullet->flags.explosive)
	{
		CObject* O = Level().Objects.net_Find(bullet->parent_id);
		if(bullet->flags.explosive)
			PlayExplodeSound(O, vEnd);
		else if (pSound && ShowMark && dist_to_cam_sqr < 10000)
		{
			Fvector position = vEnd;
			pSound->play_no_feedback(O, 0, 0.f, &position);
		}
	}

	LPCSTR ps_name = (!mtl_pair || mtl_pair->CollideParticles.empty() || bullet->flags.explosive)
		                 ? NULL
		                 : *mtl_pair->CollideParticles[::Random.randI(0, mtl_pair->CollideParticles.size())];

	if ((ps_name && ShowMark) || bullet->flags.explosive)
	{
		VERIFY2(
		...
		if (bullet->flags.explosive)
		{
			...
			shared_str& ps_name = m_ExplodeParticles[Random.randI(0, m_ExplodeParticles.size())];
			CParticlesObject* ps = CParticlesObject::Create(*ps_name, TRUE);
			ps->UpdateParent(xf, zero_vel);
			GamePersistent().ps_needtoplay.push_back(ps);
		}
		else if (ps_name && ShowMark && dist_to_cam_sqr < 10000)
		{
...
void CBulletManager::DynamicObjectHit(CBulletManager::_event& E)
{
	...
	if (!E.R.O->in_solid_state())
		return;

	if (g_clear) E.Repeated = false;
	/*if (IsGameTypeSingle())*/ E.Repeated = false; // werasik2aa not sure
...
{
	extern luabind::functor<void>* CBulletManager__ObjectHit;
	if (CBulletManager__ObjectHit)
		(*CBulletManager__ObjectHit)(bullet->section, smart_cast<CGameObject*>(R.O) ? smart_cast<CGameObject*>(R.O)->lua_game_object() : nullptr, end_point, bullet->dir, *GMLib.GetMaterialByIdx(target_material)->m_Name, bullet->speed, bullet->weapon_id);
