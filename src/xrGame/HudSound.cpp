                                             ...
                                             bool hud_mode, bool looped, u8 index, float volume_mult)
{
	...
	extern luabind::functor<luabind::object>* COnBeforePlayHudSound;
	if (COnBeforePlayHudSound)
	{
		...
		if (parent_game_object)
		{
			...
			luabind::object output = (*COnBeforePlayHudSound)(alias, parent_lua_game_object);
			if (output && output.type() == LUA_TTABLE)
			{
				...
				{
					shared_str new_alias; new_alias.printf("%s|%s", section, line);
...
void HUD_SOUND_COLLECTION_LAYERED::UpdateAllSoundsPositions(Fvector& P)
{
	xr_vector<HUD_SOUND_COLLECTION>::iterator it = m_sound_items.begin();
	xr_vector<HUD_SOUND_COLLECTION>::iterator it_e = m_sound_items.end();
	...
	for (; it != it_e; ++it)
	{
		xr_vector<HUD_SOUND_ITEM>::iterator sit = it->m_sound_items.begin();
		xr_vector<HUD_SOUND_ITEM>::iterator sit_e = it->m_sound_items.end();
		for (; sit != sit_e; ++sit)
			sit->set_position(P);
	}
}
