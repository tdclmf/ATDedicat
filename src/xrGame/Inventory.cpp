...
void CInventory::Update()
{
	if (OnServer())
	{
		if (m_iActiveSlot != m_iNextActiveSlot)
		{
			...
			extern luabind::functor<void>* CActor_OnChangedSlot;
			...
			{
				(*CActor_OnChangedSlot)(m_iActiveSlot, obj, prev_slot, prev_obj);
			}
		}
...
u32 CInventory::BeltWidth() const
{
	...
	extern luabind::functor<int>* CInventory_BeltWidth;
	if (CInventory_BeltWidth)
	{
		artefact_count = (*CInventory_BeltWidth)(artefact_count);
	}
...
{
	extern luabind::functor<bool>* CInventory_ItemAvailableToTrade;
	...
	for (TIItemContainer::const_iterator it = m_ruck.begin(); m_ruck.end() != it; ++it)
	{
		...
		if (!for_trade || pIItem->CanTrade())
		{
			if (bOverride)
			{
				if (CInventory_ItemAvailableToTrade)
				{
					if (!(*CInventory_ItemAvailableToTrade)(m_pOwner->cast_game_object()->lua_game_object(),
