...
{
	CPhysicsShellHolder* sh = O->cast_game_object() ? O->cast_game_object()->cast_physics_shell_holder() : NULL;
...
{
	CInventoryItem* item = O->cast_game_object() ? O->cast_game_object()->cast_inventory_item() : NULL;
	CInventoryOwner* inventory_owner = O->cast_game_object() ? O->cast_game_object()->cast_inventory_owner() : NULL;
...
{
	CCustomZone* custom_zone = O->cast_game_object() ? O->cast_game_object()->cast_custom_zone() : NULL;
	if (!custom_zone)
		return (true);
...
CActor::pickup_result_t CActor::PickupModeUpdate()
{
		...
		m_pUsableObject->nonscript_usable() && !Level().m_feel_deny.is_object_denied(m_pObjectWeLookingAt))
	{
		...
		extern luabind::functor<bool>* actor_on_item_before_pickup;
		if (actor_on_item_before_pickup)
		{
			callback_handled = true;
			bool allow_pickup = (*actor_on_item_before_pickup)(m_pObjectWeLookingAt->lua_game_object());
			if (!allow_pickup)
	...
	if (CurrentGameUI() && !CurrentGameUI()->GetPdaMenu().IsShown())
		DrawPickupItems();
	else
...
{
	if (Level().CurrentViewEntity() != this && !CurrentGameUI()) return;
	...
	for (u32 o_it = 0; o_it < ISpatialResult.size(); o_it++)
	{
		...
		CInventoryItem* pIItem = spatial->dcast_CObject()->cast_game_object() ? spatial->dcast_CObject()->cast_game_object()->cast_inventory_item() : NULL;

		if (0 == pIItem) continue;
		if (pIItem->object().H_Parent() != NULL) continue;
		if (!pIItem->CanTake()) continue;
		if (pIItem->object().cast_explosive_rocket()) continue;

		CGrenade* pGrenade = spatial->dcast_CObject()->cast_game_object() ? spatial->dcast_CObject()->cast_game_object()->cast_grenade() : NULL;
		if (pGrenade && !pGrenade->Useful()) continue;

		CMissile* pMissile = spatial->dcast_CObject()->cast_game_object() ? spatial->dcast_CObject()->cast_game_object()->cast_missile() : NULL;
	...
	{
		extern luabind::functor<void>* CPickupModeUpdate_COD;
		if (CPickupModeUpdate_COD)
		{
			(*CPickupModeUpdate_COD)(pNearestItem && pNearestItem->cast_game_object() ? pNearestItem->cast_game_object()->lua_game_object() : nullptr);
		}
	}
	...
	if (pNearestItem && m_bPickupMode)
	{
		...
		if (!pickup_result.callback_handled)
		{
			...
			if (actor_on_item_before_pickup)
			{
				bool allow_pickup = (*actor_on_item_before_pickup)(pNearestItem->cast_game_object()->lua_game_object());
		...
		CUsableScriptObject* pUsableObject = pNearestItem->cast_game_object() ? pNearestItem->cast_game_object()->cast_usable_script_object() : nullptr;
...
void CActor::PickupInfoDraw(CObject* object)
{
	...
	CInventoryItem* item = object->cast_game_object() ? object->cast_game_object()->cast_inventory_item() : NULL;
	if (!item) return;
...
void CActor::Feel_Grenade_Update( float rad )
{
	...
	for ( ; it_b != it_e; ++it_b )
	{
		...
		CGrenade* grn = (*it_b)->cast_game_object() ? (*it_b)->cast_game_object()->cast_grenade() : NULL;
