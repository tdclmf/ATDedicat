...
{
	u64 _cur_time = OnClient() ? Level().GetGameTime() : Level().timeServer();
...
CWound* CEntityCondition::ConditionHit(SHit* pHDS)
{
	...
	extern luabind::functor<void>* CBeforeHitAfterCalcs;
	...
	case ALife::eHitTypeTelepatic:
		...
		// demonized: add lua callback before hit but after calculations
		if (CBeforeHitAfterCalcs)
		{
			applyBeforeHitAfterCalcsCallback(m_object, *CBeforeHitAfterCalcs, pHDS, hit_power, m_fHealthHitPart);
		}
	...
	case ALife::eHitTypeBurn:
		...
		if (CBeforeHitAfterCalcs)
		{
			applyBeforeHitAfterCalcsCallback(m_object, *CBeforeHitAfterCalcs, pHDS, hit_power, m_fHealthHitPart * m_fHitBoneScale);
		}
	...
	case ALife::eHitTypeRadiation:
		...
		if (CBeforeHitAfterCalcs)
		{
			applyBeforeHitAfterCalcsCallback(m_object, *CBeforeHitAfterCalcs, pHDS, hit_power, 1);
		}
