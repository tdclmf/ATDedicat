...
void CBinocularsVision::Update()
{
	...
	for (; v_it != vVisibles.end(); ++v_it)
	{
		...
		found = std::find_if(m_active_objects.begin(), m_active_objects.end(), [object_](SBinocVisibleObj* other) {return other->m_object == object_; });
	...
	// death or invis
	for_each(m_active_objects.begin(), m_active_objects.end(), [](SBinocVisibleObj * _it)
	{
		if (!_it) return;
		...
		Fvector opos = Fvector(object_->Position());
		if (!EA->g_Alive() ||
			(0 > opos.sub(Actor()->Position()).dotproduct(Device.vCameraDirection)))
...
void CBinocularsVision::remove_links(CObject* object)
{
	VIS_OBJECTS::iterator I = std::find_if(m_active_objects.begin(), m_active_objects.end(),
		[object](SBinocVisibleObj* other) {return other->m_object == object; });
	if (I == m_active_objects.end())
		return;
	SBinocVisibleObj* visObj = (*I);
