...
void MultipacketSender::SendPacket(const void* packet_data, u32 packet_sz, u32 steamFlags, u32 timeout)
{
	_buf_cs.Enter();

	Buffer* buf = &_buf;

	// Определяем, является ли пакет надежным (Reliable)
	BOOL bReliable = (steamFlags & k_nSteamNetworkingSend_Reliable) ||
		(steamFlags & k_nSteamNetworkingSend_ReliableNoNagle);

	// Определяем, нужно ли отправлять немедленно
	BOOL bSendImmediately = (steamFlags & k_nSteamNetworkingSend_NoDelay) != 0;

	switch (psNET_GuaranteedPacketMode)
	{
	case NET_GUARANTEEDPACKET_IGNORE:
	{
		// Игнорируем надежность - всегда unreliable
		bReliable = FALSE;
		steamFlags &= ~(k_nSteamNetworkingSend_Reliable | k_nSteamNetworkingSend_ReliableNoNagle);
	...
	// Проверяем, изменился ли тип пакета (reliable/unreliable)
	BOOL bOldReliable = (buf->last_flags & k_nSteamNetworkingSend_Reliable) ||
		(buf->last_flags & k_nSteamNetworkingSend_ReliableNoNagle);

	if ((buf->buffer.B.count + packet_sz + sizeof(u16) >= NET_PacketSizeLimit)
		|| (bOldReliable != bReliable)  // Если тип пакета изменился
		|| bSendImmediately  // Если нужно отправить немедленно
		)
	{
...
void MultipacketReceiver::ReceivePacket(const void* packet_data, u32 packet_sz, u32 param) {
	MultipacketHeader* header = (MultipacketHeader*)packet_data;
	u8 data[MaxMultipacketSize];

	if (header->tag != NET_TAG_MERGED && header->tag != NET_TAG_NONMERGED) {
		_Receive(packet_data, packet_sz, param);
		return;
	}
