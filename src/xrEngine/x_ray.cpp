...
#include <mmsystem.h> // Для timeBeginPeriod/timeEndPeriod
#pragma comment(lib, "winmm.lib") // Подключение библиотеки для таймеров

// Определяем пользовательское сообщение
#define WM_APP_UPDATE (WM_APP + 1)

static HBITMAP hLogoBitmap = NULL;
static bool bAnimationRunning = true;
HWND logoWindow = NULL;

bool isDragging = false;
POINT dragStartPos; // Начальная позиция курсора при клике
RECT windowStartRect; // Начальные координаты окна

// Структура для хранения символов матрицы
struct MatrixSymbol {
	char character;
	int alpha;
	int yOffset;
...
static MatrixSymbol matrixSymbols[40][30] = { 0 };
static int columnPositions[40] = { 0 };
static bool matrixInitialized = false;

// Функция для инициализации матричного дождя
void InitializeMatrixEffect(HWND hwnd) {
	...
	for (int i = 0; i < 40; i++) {
		columnPositions[i] = -rand() % (rc.bottom + 30 * 15);
		for (int j = 0; j < 30; j++) {
			matrixSymbols[i][j].character = rand() % 94 + 33;
			matrixSymbols[i][j].alpha = 0;
			matrixSymbols[i][j].yOffset = j * 15;
		}
	}
...
// Функция для обновления матричного дождя
void UpdateMatrixEffect(HWND hwnd) {
	RECT rc;
	GetClientRect(hwnd, &rc);
	...
	for (int i = 0; i < 40; i++) {
		...
		// Проверяем, полностью ли колонка ушла за нижнюю границу
		if (columnPositions[i] - 30 * 15 > rc.bottom) {
			columnPositions[i] = -rand() % (30 * 15);
			for (int j = 0; j < 30; j += 2 + rand() % 3) {
				matrixSymbols[i][j].character = rand() % 94 + 33;
				matrixSymbols[i][j].alpha = 0;
			}
		}
		...
		for (int j = 0; j < 30; j++) {
			// Плавное появление
			if (matrixSymbols[i][j].alpha < 255) {
				matrixSymbols[i][j].alpha += 15;
				if (matrixSymbols[i][j].alpha > 255) matrixSymbols[i][j].alpha = 255;
			}

			// Плавное исчезновение для хвоста
			if (j > 0 && matrixSymbols[i][j - 1].alpha > 0) {
				matrixSymbols[i][j].alpha = matrixSymbols[i][j - 1].alpha - 10;
				if (matrixSymbols[i][j].alpha < 0) matrixSymbols[i][j].alpha = 0;
			}
		}
	}
}

// Функция для рисования матричного дождя
void DrawMatrixEffect(HDC hdc, HWND hwnd) {
	RECT rc;
	GetClientRect(hwnd, &rc);

	int columnWidth = rc.right / 40;

	for (int i = 0; i < 40; i++) {
		int x = i * columnWidth + (columnWidth / 4);

		for (int j = 0; j < 30; j++) {
			int y = columnPositions[i] - matrixSymbols[i][j].yOffset;

			if (matrixSymbols[i][j].alpha > 0 && y >= 0 && y < rc.bottom) {
				// Создаем цвет с альфа-каналом
				COLORREF color;
				if (j == 0) {
					color = RGB(255, 255, 0); // Ярко-желтый для первого символа
				}
				else {
					int green = 200 - j * 5;
					if (green < 0) green = 0;
					if (green > 255) green = 255;
					color = RGB(0, green, 0); // Зеленый градиент для хвоста
				}
				...
				// Рисуем символ
				WCHAR wchar[2] = { (WCHAR)matrixSymbols[i][j].character, 0 };
				TextOutW(hdc, x, y, wchar, 1);
			}
		}
	}
}
...
static INT_PTR CALLBACK logDlgProc(HWND hw, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	{
	case WM_NCHITTEST: {
		LRESULT hit = DefWindowProc(hw, msg, wp, lp);
	...
	case WM_INITDIALOG:
	{
		...
		hLogoBitmap = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_BITMAP1));
		...
		// Устанавливаем битмап
		SendDlgItemMessage(hw, IDC_STATIC_LOGO, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hLogoBitmap);
		...
		// Инициализируем матричный эффект
		InitializeMatrixEffect(hw);

		return TRUE;
	}

	case WM_CTLCOLORSTATIC:
		if ((HWND)lp == GetDlgItem(hw, IDC_STATIC_LOGO)) {
	...
	case WM_PAINT:
	{
		...
		FillRect(hMemDC, &rc, hBlackBrush);

		// Рисуем логотип
		if (hLogoBitmap) {
			HDC hLogoDC = CreateCompatibleDC(hMemDC);
			HBITMAP hOldLogo = (HBITMAP)SelectObject(hLogoDC, hLogoBitmap);

			BITMAP bm;
			GetObject(hLogoBitmap, sizeof(BITMAP), &bm);
			BitBlt(hMemDC, 0, 0, bm.bmWidth, bm.bmHeight, hLogoDC, 0, 0, SRCCOPY);

			SelectObject(hLogoDC, hOldLogo);
			DeleteDC(hLogoDC);
		}
		...
		DrawMatrixEffect(hMemDC, hw);

		// Рисуем версию
		SetTextColor(hMemDC, RGB(0, 255, 0));
		WCHAR versionStr[256];
		swprintf_s(versionStr, 256, L"AOE v%d.%d.%d", GET_MAJOR(AOE_Version), GET_MINOR(AOE_Version), GET_PATCH(AOE_Version));
		TextOutW(hMemDC, bitmapWidth - 110, bitmapHeight - 15, versionStr, wcslen(versionStr));
	...
	while (bAnimationRunning)
	{
		if (IsWindow(hwnd))
		{
			if (IsWindowVisible(hwnd))
				PostMessage(hwnd, WM_APP_UPDATE, 0, 0);
...
void ShowLogoPicture()
{
	PROF_EVENT("ShowLogoPicture");
	thread_spawn([](void*)
		{
			PROF_THREAD("LogoPicture_Thread");
			logoWindow = CreateDialog(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_STARTUP),
				0, logDlgProc);

			// Устанавливаем стиль "неактивного" окна
			SetWindowLong(logoWindow, GWL_EXSTYLE, GetWindowLong(logoWindow, GWL_EXSTYLE) |
				WS_EX_NOACTIVATE | WS_VISIBLE | WS_POPUP | WS_BORDER);

			HWND logoPicture = GetDlgItem(logoWindow, IDC_STATIC_LOGO);
			RECT logoRect;
			GetWindowRect(logoPicture, &logoRect);

			SetWindowPos(logoWindow, HWND_TOPMOST, 0, 0,
				logoRect.right - logoRect.left, logoRect.bottom - logoRect.top,
			...
			HANDLE hThread = CreateThread(NULL, 0, AnimationThread, logoWindow, 0, NULL);

			// Основной цикл сообщений
			MSG msg;
			while (GetMessage(&msg, logoWindow, 0, 0))
			{
			...
			// Ожидаем завершение потока анимации
...
{
	g_dedicated_server = strstr(lpCmdLine, "-dedicated");
	int lj_pool_size = luaL_create_allocated_block();
	PROF_THREAD("Window UI thread");
	PROF_EVENT("StartEngine");
...
#define STALKER_PRESENCE_MUTEX "Local\\STALKER-COP"
	...
	Msg("LuaJit pool size: %d mb", lj_pool_size);
	InitSettings();
	Msg(XRAY_MONOLITH_VERSION);

	{
		PROF_EVENT("gamedata_ListFiles");
		FS_FileSet fset;
...
{
	PROF_EVENT("CApplication::CApplication");
	ll_dwReference = 0;
...
void CApplication::OnEvent(EVENT E, u64 P1, u64 P2)
{
	if (E == eQuit)
	{
		...
		PostThreadMessage(Device.uiThreadID, WM_QUIT, 0, 0);
...
{
	PROF_EVENT("CApplication::OnFrame");
