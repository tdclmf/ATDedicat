...
void CRenderDevice::message_loop()
{
	...
	while (GetMessage(&msg, NULL, 0U, 0U))
	{
		...
		{
			PostQuitMessage(0); // Отправляем WM_QUIT
			break;
		}
	}
...
private:
	...
	// Для отслеживания медианы используем более простой подход
...
public:
	...
	void CalculateSmoothedDelta(float& current_delta)
	{
		...
		median_dirty = true; // Помечаем, что медиану нужно пересчитать

		current_delta = getFilteredAverage();
	}

	// Быстрое вычисление медианы с кэшированием
	float getMedian() const
	{
		...
		// Создаем копию для сортировки только нужной части
	...
	// Оптимизированное вычисление отфильтрованного среднего
	float getFilteredAverage() const
	{
		...
		if (valid_count > 0) {
			...
			// Если все значения - выбросы, возвращаем медиану
			return median;
		}
	}
...
private:
	...
	{
		size_t count = buffer_filled ? BUFFER_SIZE : current_index;
		if (count < 5) return median * 2.0f; // Базовый порог
		...
		float mad = deviations[count / 2]; // Медиана отклонений

		// Динамический порог: медиана + 3 * MAD
