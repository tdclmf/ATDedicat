...
extern ENGINE_API u32 g_current_renderer;
...
public:
	CCC_r2(LPCSTR N) : inherited(N, &g_current_renderer, NULL) { g_current_renderer = 0; };
	...
	virtual void Execute(LPCSTR args)
	{
		...
		for (int i = 0; vid_quality_token[i].name; i++)
			if (i == g_current_renderer)
				renderer_name = vid_quality_token[i].name;

		BOOL isR1 = !strcmp("renderer_r1", renderer_name);
		BOOL isR2 = !strcmp("renderer_r2", renderer_name);
		BOOL isR2a = !strcmp("renderer_r2a", renderer_name);
		BOOL isR3 = !strcmp("renderer_r3", renderer_name);
		BOOL isR4 = !strcmp("renderer_r4", renderer_name);

		psDeviceFlags.set(rsR1, isR1 || g_dedicated_server);
		psDeviceFlags.set(rsR2, isR2 || isR2a);
		psDeviceFlags.set(rsR3, isR3);
		psDeviceFlags.set(rsR4, isR4);

		r2_sun_static = isR1 || isR2;
		r2_advanced_pp = isR2a || isR3 || isR4;
	}
...
void CCC_Register()
{
	...
	CMD4(CCC_Integer, "ignore_error_window", &Core.ignore_error_window, 0, 1);
