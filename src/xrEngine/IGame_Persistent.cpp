...
{
	PROF_EVENT("IGame_Persistent::IGame_Persistent");
...
{
	PROF_EVENT("IGame_Persistent::OnFrame");
#ifndef _EDITOR
	...
	{
		xrCriticalSection::raii guard(&Device.ldque_cs);
		for (IRender_Light* L : Device.v_all_lights_dque)
			xr_delete(L);
		Device.v_all_lights_dque.clear();
	}
	...
	//string256 str;
	//xr_sprintf(str,sizeof(str), "ps_active: %d ps_destroy: %d", ps_active.size(), ps_destroy.size());
	//g_pGameLevel->d2dtext_renderer({500,50}, str);
#endif
	static std::unordered_set<CPS_Instance*> ps_active_mt;
	ps_active_mt.reserve(g_pGamePersistent->ps_active.size());
	ps_active_mt = g_pGamePersistent->ps_active;
	static DWORD this_thread_id = GetCurrentThreadId();
	if (g_pGameLevel && g_pGameLevel->bReady && g_pGameLevel->CurrentEntity() && !Device.Paused() && !Device.dwPrecacheFrame)
	{
		Device.parallel_render_tasks.run([]()
		{
			if (this_thread_id != GetCurrentThreadId()) { PROF_THREAD("X-Ray PPL Thread") }
			Core.InitializeCOM();
			PROF_EVENT("UpdateParticles");
			u32 dwTime = Device.dwTimeGlobal;
			for (CPS_Instance* PS : ps_active_mt)
			{
				PS->shedule_Update(dwTime-PS->dwTimeOfLastExecute);
				PS->dwTimeOfLastExecute = dwTime;
			}
		});
	}
}
...
{
	PROF_EVENT("IGame_Persistent::GrassBendersUpdateAnimations");
	for (int idx = 1; idx < ps_ssfx_grass_interactive.y + 1; idx++)
	{
		if (grass_shader_data.id[idx] != NULL)
		{
