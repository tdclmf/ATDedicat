...
{
	PROF_EVENT("CGameMtlLibrary::Load");
	string_path name;
	...
	if (FS.exist(materialsLtxName, _game_data_, "materials\\materials", ".ltx"))
	{
		...
		CInifile materialsLtx = CInifile(materialsLtxName, TRUE);
		for (CInifile::Sect* sec : materialsLtx.sections())
		{
			SGameMtl* M = NULL;
	...
	if (OBJ)
	{
		...
		for (IReader* O = OBJ->open_chunk_iterator(count); O; O = OBJ->open_chunk_iterator(count, O))
		{
			material_pairs.emplace_back(this);
			material_pairs.back().Load(*O);
		}
		OBJ->close();
	}
...
#endif
		int biggestId = biggestIdStart;
		CInifile materialsPairsLtx = CInifile(materialPairsLtxName, TRUE);
		for (CInifile::Sect* sec : materialsPairsLtx.sections())
		{
			SGameMtlPair* M = NULL;
			...
			if (material == material_pairs.end())
			{
				if (biggestId == biggestIdStart)
				{
					for (SGameMtlPair& m : material_pairs) {
						if (m.ID > biggestId) {
							biggestId = m.ID;
						}
					}
			...
			if (materialsPairsLtx.line_exist(sec->Name, "breaking_sounds"))
			{
				LPCSTR s = materialsPairsLtx.r_string(sec->Name, "breaking_sounds");
				M->BreakingSoundsStr = s ? s : "";
				M->OwnProps.set(SGameMtlPair::flBreakingSounds, 1);
				M->CreateSoundsImpl(M->BreakingSounds, s);
			}
			if (materialsPairsLtx.line_exist(sec->Name, "step_sounds"))
			{
				LPCSTR s = materialsPairsLtx.r_string(sec->Name, "step_sounds");
				M->StepSoundsStr = s ? s : "";
				M->OwnProps.set(SGameMtlPair::flStepSounds, 1);
				M->CreateSoundsImpl(M->StepSounds, s);
			}
			if (materialsPairsLtx.line_exist(sec->Name, "collide_sounds"))
			{
				LPCSTR s = materialsPairsLtx.r_string(sec->Name, "collide_sounds");
				M->CollideSoundsStr = s ? s : "";
				M->OwnProps.set(SGameMtlPair::flCollideSounds, 1);
				M->CreateSoundsImpl(M->CollideSounds, s);
			}
			if (materialsPairsLtx.line_exist(sec->Name, "collide_particles"))
			{
				LPCSTR s = materialsPairsLtx.r_string(sec->Name, "collide_particles");
				M->CollideParticlesStr = s ? s : "";
				M->OwnProps.set(SGameMtlPair::flCollideParticles, 1);
				M->CreateParticlesImpl(M->CollideParticles, s);
			}
			if (materialsPairsLtx.line_exist(sec->Name, "collide_marks"))
			{
				LPCSTR s = materialsPairsLtx.r_string(sec->Name, "collide_marks");
