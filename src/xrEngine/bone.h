...
extern ENGINE_API xrCriticalSection cs_bc;
// callback
...
public:
	...
	// methods
public:
	IC BoneCallback _BCL callback() { xrCriticalSection::raii guard(&cs_bc); return Callback; }
	IC void* _BCL callback_param() { xrCriticalSection::raii guard(&cs_bc); return Callback_Param; }
	IC BOOL _BCL callback_overwrite() { xrCriticalSection::raii guard(&cs_bc); return Callback_overwrite; } // performance hint - don't calc anims
	IC u32 _BCL callback_type() { xrCriticalSection::raii guard(&cs_bc); return Callback_type; }
public:
	IC void _BCL construct();

	void _BCL set_callback(u32 Type, BoneCallback C, void* Param, BOOL overwrite = FALSE)
	{
		xrCriticalSection::raii guard(&cs_bc);
		Callback = C;
	...
	void _BCL set_callback_overwrite(BOOL v) { xrCriticalSection::raii guard(&cs_bc); Callback_overwrite = v; }
