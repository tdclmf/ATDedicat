...
void CEffect_Rain::OnFrame()
{
	...
	float rfactor = g_pGamePersistent->Environment().CurrentEnv->rain_density;
	if (rfactor < EPS_L) return;
	...
	Device.parallel_render_tasks.run([=]
	{
		...
		desired_items = iFloor(0.01f * (1.f + rfactor * 99.0f) * float(rain_max_particles));
		if (current_items < desired_items)
			current_items += desired_items - current_items;

		float b_radius_wrap_sqr = _sqr((rain_radius * 1.5f));
		if (items.size() < current_items)
		{
			while (items.size() < current_items)
			{
				Item one;
