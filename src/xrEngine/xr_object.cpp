...
void CObject::cNameVisual_set(shared_str N, bool destroy)
{
	// check if equal
	if (*N && *NameVisual)
	...
	if (*N && N[0])
	{
		...
		IKinematics* new_k = renderable.visual ? renderable.visual->dcast_PKinematics() : NULL;
