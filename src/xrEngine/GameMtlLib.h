...
public:
	...
	{
		auto material = std::find_if(materials.begin(), materials.end(), [&name](const SGameMtl& m) {
			return xr_strcmp(m.m_Name, shared_str(name)) == 0;
			});
		return material;
	}
	...
	{
		auto material = std::find_if(materials.begin(), materials.end(), [&name](const SGameMtl& m) {
			return xr_strcmp(m.m_Name, name) == 0;
			});
		return material;
	}
	...
	IC u16 GetMaterialIdx(int ID)
	{
		...
		if (materials.end() == it)
		{
			//Msg("~Warning material: ID[%d] not found in library", ID);
			it = materials.begin();
		}
	...
	IC u16 GetMaterialIdx(LPCSTR name)
	{
		...
		if (materials.end() == it)
		{
			//Msg("~Warning material: %s not found in library", name);
			it = materials.begin();
		}
		return (u16)(it - materials.begin());
	}
	...
	IC SGameMtl* GetMaterialByIdx(u16 idx)
	{
		...
		{
			//Msg("~Warning material: idx[%d] not found in library", idx);
			return &materials[0];
		}
