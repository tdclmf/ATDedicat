...
PROTECT_API void CRenderDevice::Initialize()
{
	...
	if (m_hWnd == NULL)
	{
		...
		if (!RegisterHotKey(m_hWnd, 1, MOD_CONTROL | MOD_ALT, VK_OEM_5))
			if (!RegisterHotKey(m_hWnd, 1, MOD_CONTROL | MOD_ALT, VK_OEM_6))
				if (!RegisterHotKey(m_hWnd, 1, MOD_CONTROL | MOD_ALT, VK_OEM_2))
					if (!RegisterHotKey(m_hWnd, 1, MOD_CONTROL | MOD_ALT, VK_OEM_7))
						if (!RegisterHotKey(m_hWnd, 1, MOD_CONTROL | MOD_ALT, VK_OEM_1))
							if (!RegisterHotKey(m_hWnd, 1, MOD_CONTROL | MOD_ALT, VK_OEM_PERIOD))
								if (!RegisterHotKey(m_hWnd, 1, MOD_CONTROL | MOD_ALT, VK_OEM_4))
									if (!RegisterHotKey(m_hWnd, 1, MOD_CONTROL | MOD_ALT, VK_OEM_COMMA))
										Msg("~Warning Optick profiler hotkey registration failed");
	}
