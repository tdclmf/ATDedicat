...
public:
	...
	xr_vector<fastdelegate::FastDelegate0<>> seqParallel, seqParallelPB;
...
public:
	...
	// Multi-threading
	xrCriticalSection ldque_cs;
	xr_vector<IRender_Light*> v_all_lights_dque;
	concurrency::task_group all_frame_tasks, before_render_tasks, parallel_render_tasks;
	u32 uiThreadID;
	u32 mmThreadID;
	...
	ICF void remove_from_seq_parallelpb(const fastdelegate::FastDelegate0<>& delegate)
	{
		xr_vector<fastdelegate::FastDelegate0<>>::iterator I = std::find(
			seqParallelPB.begin(),
			seqParallelPB.end(),
		...
		if (I != seqParallelPB.end())
			seqParallelPB.erase(I);
	}
