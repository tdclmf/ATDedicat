...
HANDLE g_FilesChache = INVALID_HANDLE_VALUE;

static bool InitFilesChache()
{
	g_FilesChache = CreateFileA(
		"FilesChache.fch",
		FILE_APPEND_DATA,
		FILE_SHARE_READ,
		nullptr,
	...
	if (g_FilesChache != INVALID_HANDLE_VALUE)
	{
		SetFilePointer(g_FilesChache, 0, nullptr, FILE_END);
		return true;
	}
	return false;
}
...
static void WriteFilesChacheLine(LPCSTR text)
{
	if (g_FilesChache == INVALID_HANDLE_VALUE) return;

	std::string line = std::string(text) + newline;

	DWORD written;
	WriteFile(g_FilesChache, line.data(), line.size(), &written, nullptr);
}

static void CleanupFilesChache()
{
	if (g_FilesChache != INVALID_HANDLE_VALUE)
	{
		CloseHandle(g_FilesChache);
		g_FilesChache = INVALID_HANDLE_VALUE;
	}
}
std::vector<std::string> g_FilesChachev;
static void ReadFilesChache()
{
	HANDLE hFile = CreateFileA("FilesChache.fch", GENERIC_READ, 0, nullptr, OPEN_EXISTING, 0, nullptr);
	if (hFile == INVALID_HANDLE_VALUE) return;

	DWORD size = GetFileSize(hFile, nullptr);
	std::string content(size, '\0');
	ReadFile(hFile, (LPVOID)content.data(), size, nullptr, nullptr);

	size_t pos = 0;
	while ((pos = content.find("\r\n")) != std::string::npos)
	{
		g_FilesChachev.push_back(content.substr(0, pos));
...
void CLocatorAPI::Register(LPCSTR name, u32 vfs, u32 crc, u32 ptr, u32 size_real, u32 size_compressed, u32 modif)
{
    ...
    char* temp = (char*)desc.name.c_str();
    u32 vfs_id = vfs;
    ...
    for (char* p = temp + len - 1; p > temp; --p)
    {
        if (*p == '\\' || *p == '/')
        {
            ...
            *(p + 1) = '\0';
            ...
            {
                file folder_desc;
                folder_desc.name = temp;
                folder_desc.vfs = vfs_id;
                folder_desc.ptr = 0;
                folder_desc.size_real = 0;
                folder_desc.size_compressed = 0;
                folder_desc.modif = u32(-1);

				std::pair<files_it, bool> I = m_files.insert(folder_desc);
				R_ASSERT(I.second);
            }
...
bool CLocatorAPI::Recurse(LPCSTR path)
{
	...
	if (newSize > oldSize)
	{
		for (size_t i = oldSize; i < newSize; i++)
		{
			...
			xr_strcat(N, chache.fileName);
			xr_strlwr(N);

			if (chache.fattr & FILE_ATTRIBUTE_DIRECTORY)
			{
				if (bNoRecurse) continue;
...
void CLocatorAPI::_initialize(u32 flags, LPCSTR target_folder, LPCSTR fs_name)
{
	...
	static float FileSystemsec = 0.0f;
	FileSystemsec = t.GetElapsed_sec();
	Msg("Init FileSystem %f sec", FileSystemsec);
	//thread_spawn([](void*)
	//{
	//	shared_str elapsed_str; elapsed_str.printf("Init FileSystem %f sec", FileSystemsec);
...
xr_vector<char*>* CLocatorAPI::file_list_open(const char* _path, u32 flags)
{
	...
	{
		const file& entry = *I;
		if (0 != strncmp(entry.name.c_str(), N, base_len)) break; // end of list
		const char* end_symbol = entry.name.c_str() + xr_strlen(entry.name.c_str()) - 1;
		if ((*end_symbol) != '\\')
		{
			// file
			if ((flags & FS_ListFiles) == 0) continue;

			const char* entry_begin = entry.name.c_str() + base_len;
...
int CLocatorAPI::file_list(FS_FileSet& dest, LPCSTR path, u32 flags, LPCSTR mask)
{
	...
	for (++I; I != m_files.end(); ++I)
	{
		const file& entry = *I;
		if (0 != strncmp(entry.name.c_str(), N, base_len)) break; // end of list
		LPCSTR end_symbol = entry.name.c_str() + xr_strlen(entry.name.c_str()) - 1;
		if ((*end_symbol) != '\\')
		{
			// file
			if ((flags & FS_ListFiles) == 0) continue;
			LPCSTR entry_begin = entry.name.c_str() + base_len;
			if ((flags & FS_RootOnly) && strstr(entry_begin, "\\")) continue; // folder in folder
			// check extension
			if (b_mask)
			{
...
CLocatorAPI::files_it CLocatorAPI::file_find_it(LPCSTR fname)
{
	...
	return m_files.find({ fname,0,0,0,0,0,0 });
}
...
BOOL CLocatorAPI::dir_delete(LPCSTR path, LPCSTR nm, BOOL remove_files)
{
	...
	if (I != m_files.end())
	{
		...
		for (; I != m_files.end();)
		{
			...
			I = cur_item;
			I++;
			if (0 != strncmp(entry.name.c_str(), fpath, base_len)) break; // end of list
			const char* end_symbol = entry.name.c_str() + xr_strlen(entry.name.c_str()) - 1;
			if ((*end_symbol) != '\\')
			{
				// const char* entry_begin = entry.name+base_len;
				if (!remove_files) return FALSE;
				unlink(entry.name.c_str());
	...
	{
		const char* end_symbol = r_it->name.c_str() + xr_strlen(r_it->name.c_str()) - 1;
		if ((*end_symbol) == '\\')
		{
...
void CLocatorAPI::rescan_path(LPCSTR full_path, BOOL bRecurse)
{
	...
	for (; I != m_files.end();)
	{
		...
		I = cur_item;
		I++;
		if (0 != strncmp(entry.name.c_str(), full_path, base_len)) break; // end of list
