...
{
	DialogData();
	DialogData(std::string ttl, std::string msg);
    ...
    int lineHeight = 0;
    int result = 0; // 0 = OK, 1 = Отмена
    void* hBackgroundBrush = NULL;
    void* hButtonBrush = NULL;
    void* hButtonHoverBrush = NULL;
    void* hCancelButtonBrush = NULL; // Красноватый для отмены
    void* hCancelButtonHoverBrush = NULL; // Более яркий для hover
};
...
public:
    ...
    void CalculateInitialSize(DialogData& data);
};
...
public:
    ...
    int ShowStackTrace(std::string title, bool print_stack = false);
};
