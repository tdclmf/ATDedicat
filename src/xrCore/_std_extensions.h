...
// check for: Signaling NaN, Quiet NaN, Negative infinity ( ЦINF), Positive infinity (+INF), Negative denormalized, Positive denormalized
IC BOOL _validd(const double x)
{
	int cls = _fpclass(x);
	if (cls & (_FPCLASS_SNAN + _FPCLASS_QNAN + _FPCLASS_NINF + _FPCLASS_PINF + _FPCLASS_ND + _FPCLASS_PD))
		return false;
...
{
	// ≈сли оба указател€ nullptr, считаем строки равными
	if (S1 == nullptr && S2 == nullptr)
		return 0;

	// ≈сли только S1 nullptr, считаем его "меньше"
	if (S1 == nullptr)
		return -1;

	// ≈сли только S2 nullptr, считаем его "меньше"
	if (S2 == nullptr)
		return 1;
