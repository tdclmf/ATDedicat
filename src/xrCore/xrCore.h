...
#define AOE_Version 0.55f
#define GET_MAJOR(v) ((int)(v))
#define GET_MINOR(v) ((int)((v) * 10) % 10)
#define GET_PATCH(v) ((int)((v) * 100) % 10)
#define TRY __try{
#define CATCH }__except (EXCEPTION_EXECUTE_HANDLER){}

#define TRYC(enter_code) \
    __try { \
        enter_code; \

#define CATCHC(catch_code) \
...
// ********************************************** The Core definition
static thread_local bool co_init = false;
...
public:
	...
	DWORD dwFrame;
	bool april1;
	int ignore_error_window = 0;
public:
	void _initialize(LPCSTR ApplicationName, LogCallback cb = 0, BOOL init_fs = TRUE, LPCSTR fs_fname = 0);
	void _destroy();
	IC void InitializeCOM()
	{
		if (!co_init)
		{
