...
void CParticleGroup::SItem::StartRelatedChild(CParticleEffect* emitter, LPCSTR eff_name, PAPI::Particle& m)
{
	...
	Fmatrix M;
	M.identity();
	Fvector vel; vel.sub(m.pos, m.posB); vel.div(C->m_RT_Flags.is(CParticleEffect::flRT_LiveUpdate) ? Device.fTimeDelta : C->m_Def->GetFStep());
	...
	if (E)
	{
		...
		if (E->IsPlaying())
		{
			...
			if (def.m_Flags.is(CPGDef::SEffect::flOnPlayChild) && def.m_OnPlayChildName.size())
			{
				...
				u32 p_cnt;
				E->PE.GetParticles(particles, p_cnt);
