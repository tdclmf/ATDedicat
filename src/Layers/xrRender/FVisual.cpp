...
void Fvisual::Load(const char* N, IReader* data, u32 dwFlags)
{
	...
	BOOL loaded_v = false;
	string256 d_str;
	xr_sprintf(d_str, sizeof(d_str), "!Error FVisual::Load: can't load visual %s %s %s", N ? N : "", *dbg_shader_def, *dbg_texture_def);
	if (data->find_chunk(OGF_GCONTAINER))
	{
		...
		if (data->find_chunk(OGF_FASTPATH))
		{
			...
			R_ASSERT2(m_fast->p_rm_Vertices, d_str);
			...
			R_ASSERT2(m_fast->p_rm_Indices, d_str);
