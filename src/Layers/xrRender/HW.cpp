...
#ifndef _EDITOR
	BOOL bWindowed = g_dedicated_server || !psDeviceFlags.is(rsFullscreen) || g_screenmode != 2;
...
void CHW::CreateD3D()
{
	LPCSTR _name = g_dedicated_server ? "xrd3d9-null.dll" : "d3d9.dll";
...
void CHW::updateWindowProps(HWND m_hWnd)
{
	...
	if (bWindowed)
	{
		if (m_move_window)
		{
			...
			if (!strstr(Core.Params, "-no_dialog_header") && g_screenmode != 2)
