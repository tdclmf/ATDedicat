...
public:
	...
	ICF void set_PS(ref_ps& _ps) { if(_ps) set_PS(_ps->ps, _ps->cName.c_str()); }

#if defined(USE_DX10) || defined(USE_DX11)
	ICF void set_GS(ID3DGeometryShader* _gs, LPCSTR _n = 0);
	ICF void set_GS(ref_gs& _gs) { if(_gs)set_GS(_gs->gs, _gs->cName.c_str()); }

#	ifdef USE_DX11
	ICF void set_HS(ID3D11HullShader* _hs, LPCSTR _n = 0);
	ICF void set_HS(ref_hs& _hs) { if (_hs)set_HS(_hs->sh, _hs->cName.c_str()); }

	ICF void set_DS(ID3D11DomainShader* _ds, LPCSTR _n = 0);
	ICF void set_DS(ref_ds& _ds) { if (_ds)set_DS(_ds->sh, _ds->cName.c_str()); }

	ICF void set_CS(ID3D11ComputeShader* _cs, LPCSTR _n = 0);
	ICF void set_CS(ref_cs& _cs) { if (_cs)set_CS(_cs->sh, _cs->cName.c_str()); }
#	endif
