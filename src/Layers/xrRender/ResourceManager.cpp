...
xrCriticalSection selem_cs;
ShaderElement* CResourceManager::_CreateElement(ShaderElement& S)
{
	xrCriticalSection::raii guard(&selem_cs);
...
xrCriticalSection shader_create_cs;
Shader* CResourceManager::Create(IBlender* B, LPCSTR s_shader, LPCSTR s_textures, LPCSTR s_constants, LPCSTR s_matrices)
{
	PROF_EVENT("CreateShader");
	xrCriticalSection::raii guard(&shader_create_cs);
#ifndef _EDITOR
	if (!g_dedicated_server)
#endif
...
void CResourceManager::DeferredUpload()
{
	...
	if(multithreaded_texture_loading)
	{
		static DWORD this_thread_id = GetCurrentThreadId();
		concurrency::parallel_for_each(m_textures.begin(), m_textures.end(), [](auto& it) {if (this_thread_id != GetCurrentThreadId()){PROF_THREAD("X-Ray PPL Thread"); Core.InitializeCOM();}  it.second->Load(); });
	}
	else
	{
...
void CResourceManager::DeferredUnload()
{
	...
	{
		static DWORD this_thread_id = GetCurrentThreadId();
		concurrency::parallel_for_each(m_textures.begin(), m_textures.end(), [](auto& it) {if (this_thread_id != GetCurrentThreadId()){PROF_THREAD("X-Ray PPL Thread"); Core.InitializeCOM();}  it.second->Unload(); });
	}
	else
	{
		for (map_TextureIt t = m_textures.begin(); t != m_textures.end(); t++)
