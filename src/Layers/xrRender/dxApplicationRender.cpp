...
void dxApplicationRender::load_draw_internal(CApplication& owner)
{
	...
	static CGameFont* pFontAOE = NULL;
	if (!pFontAOE)
	{
		void _InitializeFont(CGameFont * &F, LPCSTR section, u32 flags);
		_InitializeFont(pFontAOE, "ui_font_letterica16_russian", 0);
	}

	if (pFontAOE)
	{
		pFontAOE->SetColor(color_rgba(255, 142, 22, 70));
		pFontAOE->Out(float(Device.dwWidth)*0.95f, float(Device.dwHeight)*0.98f, "AOE v%d.%d.%d", GET_MAJOR(AOE_Version), GET_MINOR(AOE_Version), GET_PATCH(AOE_Version));
		pFontAOE->OnRender();
	}
