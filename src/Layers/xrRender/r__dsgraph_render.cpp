                                                    ...
                                                    Fvector& _cop, BOOL _dynamic, BOOL _precise_portals)
{
	...
	if (!_sector)
		_sector = RImplementation.detectSector(_cop);
	...
	if (_dynamic)
	{
		...
		for (u32 o_it = 0; o_it < lstRenderables.size(); o_it++)
		{
			...
			if (0 == renderable->renderable.visual) continue;
			if (0 == renderable->renderable.visual->dcast_PKinematics()) continue;
