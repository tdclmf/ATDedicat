...
//---------------------------------------------------------------------------
#include "../../xrParticles/particle_effect.h"
...
namespace PS
{
	...
	public:
		xrCriticalSection pe_cs;
		CPEDef* m_Def;
		PAPI::ParticleEffect PE;
		Fmatrix m_XFORM;
	protected:
	...
	public:
		...
		IC CPEDef* GetDefinition() { xrCriticalSection::raii guard(&pe_cs); return m_Def; }

		virtual void Play();
		virtual void Stop(BOOL bDefferedStop = TRUE);
		virtual BOOL IsPlaying() { xrCriticalSection::raii guard(&pe_cs); return m_RT_Flags.is(flRT_Playing); }

		virtual void SetHudMode(BOOL b) { xrCriticalSection::raii guard(&pe_cs); m_RT_Flags.set(flRT_HUDmode, b); }
		virtual BOOL GetHudMode() { xrCriticalSection::raii guard(&pe_cs); return m_RT_Flags.is(flRT_HUDmode); }
