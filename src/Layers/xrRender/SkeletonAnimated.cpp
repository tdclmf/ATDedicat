...
MotionID CKinematicsAnimated::ID_Cycle(shared_str N)
{
	...
	if(!motion_ID.valid())
		Msg("! MODEL: can't find cycle: ", N.c_str());
...
{
	PROF_EVENT("CKinematicsAnimated::LL_UpdateTracks");
	xrCriticalSection::raii guard(&utracks_cs);
	BlendSVecIt I, E;
	// Cycles
...
{
	PROF_EVENT("CKinematicsAnimated::UpdateTracks");
	_DBG_SINGLE_USE_MARKER;
...
{
	PROF_EVENT("CKinematicsAnimated::Load");
