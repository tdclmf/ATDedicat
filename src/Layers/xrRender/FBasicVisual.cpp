...
void dxRender_Visual::Load(const char* N, IReader* data, u32)
{
	...
	if (data->find_chunk(OGF_TEXTURE))
	{
		...
		if (0 == strcmp(fnS, "models\\xwindows"))
			xr_strcpy(fnS, sizeof(fnS), "models\\model_aref");
		dbg_shader_def = fnS;
		dbg_texture_def = fnT;
		ResetShaderTexture();
	}
...
{
	TRY
	if (s_shader && s_shader[0] && strlen(s_shader))
	{
	...
	if (s_texture && s_texture[0] && strlen(s_texture))
	{
