...
// change Equal if variables changed 
	...
	public:
		...
		const CPGDef* GetDefinition() { xrCriticalSection::raii guard(&pg_cs); return m_Def; }

		virtual void Play();
		virtual void Stop(BOOL bDefferedStop = TRUE);
		virtual BOOL IsPlaying() { xrCriticalSection::raii guard(&pg_cs); return m_RT_Flags.is(flRT_Playing); }
		...
		{
			xrCriticalSection::raii guard(&pg_cs);
			VERIFY(m_Def);
