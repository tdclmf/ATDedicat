...
void CRender::render_main(Fmatrix& m_ViewProjection, bool _fportals)
{
	...
	if (pLastSector)
	{
		...
		//!!!
		{
			...
			TRY std::sort(lstRenderables.begin(), lstRenderables.end(), pred_sp_sort); CATCH
		...
		for (u32 o_it = 0; o_it < lstRenderables.size(); o_it++)
		{
			...
			{
				// lightsource
				if (light* L = spatial ? (light*)(spatial->dcast_Light()) : NULL)
				{
					float lod = L->get_LOD();
					if (lod > EPS_L)
					{
...
void CRender::Render()
{
	...
	// Main calc
	Device.Statistic->RenderCALC.Begin();
	r_pmask(true, true, true); // enable priority "0",+ capture wmarks
