...
IC bool check_grass_shadow(light* L, CFrustum VB)
{
...
IC void hud_light_apply(FixedMAP<light*, std::pair<Fvector, Fvector>>& saved_pos, xr_vector<light*>& source)
{
	for (u32 it = 0; it < source.size(); it++)
	{
		...
		saved_pos.insertInAnyWay(L, mk_pair(L->position, L->direction));
...
IC void hud_light_restore(FixedMAP<light*, std::pair<Fvector, Fvector>>& saved_pos, xr_vector<light*>& source)
{
	for (const auto& saved : saved_pos)
	{
		...
		L->position = saved.val.first;
		L->direction = saved.val.second;
	}
}

void CRender::render_lights(light_Package& LP)
{
	static FixedMAP<light*, std::pair<Fvector, Fvector>> saved_pos; saved_pos.clear();
	//////////////////////////////////////////////////////////////////////////
	...
	{
		xr_vector<light*>& sourcesh = LP.v_shadowed;
		sourcesh.erase(std::remove_if(sourcesh.begin(), sourcesh.end(), 
		[](light* L)
		{
			L->vis_update();
			if(!L->vis.visible)
	...
	// 2. refactor - infact we could go from the backside and sort in ascending order
	{
		...
		static xr_vector<light*> refactored; refactored.clear();
	...
	while (LP.v_shadowed.size())
	{
		...
		static xr_vector<light*> L_spot_s; L_spot_s.clear();
		stats.s_used ++;
