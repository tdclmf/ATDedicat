	...
	g_pGamePersistent->LoadTitle();
	{
		...
		for (u32 i = 0; i < count; i++) // skip first shader as "reserved" one
		{
			...
			*delim = 0;
			...
			if (0 == strcmp(n_sh, "def_shaders\\def_trans_v"))
			{
				xr_strcpy(n_sh, "def_shaders\\def_aref_v");
			}

			if (0 == strcmp(n_sh, "def_shaders\\def_trans"))
			{
				if(strstr(n_tlist, "prop\\prop_mask_setka"))
				{
					xr_strcpy(n_sh, "def_shaders\\def_aref_v");
					xr_strcpy(n_tlist, "prop\\prop_mask_setka,prop\\prop_mask_setka,prop\\prop_mask_setka");
				}
			}
...
void CRender::level_Unload()
{
	...
	if (!b_loaded) return;
	Device.before_render_tasks.wait();
	u32 I;
