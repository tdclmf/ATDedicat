...
//--------------------------------------------------------------------------------------------------------------
xrCriticalSection stateshdr;
SState* CResourceManager::_CreateState(SimulatorStates& state_code)
{
	xrCriticalSection::raii guard(&stateshdr);
...
//--------------------------------------------------------------------------------------------------------------
xrCriticalSection passshdr;
SPass* CResourceManager::_CreatePass(const SPass& proto)
{
	xrCriticalSection::raii guard(&passshdr);
...
//--------------------------------------------------------------------------------------------------------------
xrCriticalSection vsshdr;
SVS* CResourceManager::_CreateVS(LPCSTR _name)
{
	xrCriticalSection::raii guard(&vsshdr);
	int skinning = -1;
	string_path name;
	...
	else
	{
		...
		if (FAILED(_hr))
		{
			Debug.fatal(DEBUG_INFO, "ERROR: Shader %s compilation failed, check your log file for additional information.", cname);
			return 0;
		}
...
//--------------------------------------------------------------------------------------------------------------
xrCriticalSection psshdr;
SPS* CResourceManager::_CreatePS(LPCSTR _name)
{
	xrCriticalSection::raii guard(&psshdr);
	string_path name;
	xr_strcpy(name, _name);
...
//--------------------------------------------------------------------------------------------------------------
xrCriticalSection gsshdr;
SGS* CResourceManager::_CreateGS(LPCSTR name)
{
	xrCriticalSection::raii guard(&gsshdr);
	LPSTR N = LPSTR(name);
...
#endif
{
	xrCriticalSection::raii guard(&rtse_cs);
...
xrCriticalSection txtrs_cs;
concurrency::task_group textures_load;
//--------------------------------------------------------------------------------------------------------------
CTexture* CResourceManager::_CreateTexture(LPCSTR _Name)
{
	xrCriticalSection::raii guard(&txtrs_cs);
	// DBG_VerifyTextures	();
	...
	else
	{
		...
		T->Preload();
		if (Device.b_is_Ready && !bDeferredLoad && !T->flags.bLoaded)
		{
			extern int multithreaded_texture_loading;
			if(multithreaded_texture_loading && !strstr(Name, "$user$"))
			{
...
//--------------------------------------------------------------------------------------------------------------
xrCriticalSection mtx_cs;
CMatrix* CResourceManager::_CreateMatrix(LPCSTR Name)
{
	xrCriticalSection::raii guard(&mtx_cs);
...
//--------------------------------------------------------------------------------------------------------------
xrCriticalSection cnst_cs;
CConstant* CResourceManager::_CreateConstant(LPCSTR Name)
{
	xrCriticalSection::raii guard(&cnst_cs);
	R_ASSERT(Name && Name[0]);
	if (0 == stricmp(Name, "$null")) return NULL;
...
xrCriticalSection ctxtl_cs;
STextureList* CResourceManager::_CreateTextureList(STextureList& L)
{
	xrCriticalSection::raii guard(&ctxtl_cs);
...
//--------------------------------------------------------------------------------------------------------------
xrCriticalSection cmtxl_cs;
SMatrixList* CResourceManager::_CreateMatrixList(SMatrixList& L)
{
	xrCriticalSection::raii guard(&cmtxl_cs);
	BOOL bEmpty = TRUE;
