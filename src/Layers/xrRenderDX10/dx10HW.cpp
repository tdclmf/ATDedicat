...
IDXGIAdapter1* SelectHighPerformanceGPU()
{
    xr_vector<GPUInfo> gpus;
    gpus.reserve(3);
    ...
    for (UINT i = 0; pFactory->EnumAdapters1(i, &pAdapter) != DXGI_ERROR_NOT_FOUND; i++)
    {
        ...
        pAdapter->GetDesc(&info.desc);

        info.dedicatedMemory = info.desc.DedicatedVideoMemory;
        info.sharedMemory = info.desc.SharedSystemMemory;

        Msg("* GPU %S", info.desc.Description);
    ...
    GPUInfo* bestAdapter = nullptr;
    UINT64 maxMemory = 0;

    for (GPUInfo& gpu : gpus)
    {
        if (gpu.desc.VendorId == 0x1414 && gpu.desc.DeviceId == 0x8c) // Microsoft
            continue;

        if (gpu.dedicatedMemory > maxMemory)
        {
            maxMemory = gpu.dedicatedMemory;
            bestAdapter = &gpu;
        }
    }
    ...
    Msg("* Selected GPU %S", bestAdapter->desc.Description);
    Msg("* Video memory: %d M", bestAdapter->desc.DedicatedVideoMemory / (1024 * 1024));
    Msg("\n");
    HW.Caps.id_vendor = bestAdapter->desc.VendorId;
    HW.Caps.id_device = bestAdapter->desc.DeviceId;

    return bestAdapter->adapter;
}
//////////////////////////////////////////////////////////////////////
...
void CHW::CreateD3D()
{
    ...
    */
    ...
    if (!m_pAdapter) {
#if defined(USE_DX11)
        m_pAdapter = SelectHighPerformanceGPU();
#elif defined(USE_DX10)
        pFactory->EnumAdapters(0, &m_pAdapter);
#endif
    }
...
void CHW::CreateDevice(HWND hwnd, bool move_window)
{
   ...
   // HRESULT R;
    ...
    R_CHK(D3D11CreateDevice(
        m_pAdapter,
        m_pAdapter ? D3D_DRIVER_TYPE_UNKNOWN : D3D_DRIVER_TYPE_HARDWARE,
        nullptr,
        create_device_flags,
        pFeatureLevels,
        1,
    ...
    R_CHK(pDevice->QueryInterface(__uuidof(IDXGIDevice1), (void**)&dxgiDevice));
    dxgiDevice->SetMaximumFrameLatency(2);
    dxgiDevice->SetGPUThreadPriority(7);
    _RELEASE(dxgiDevice);

    ID3D11Multithread* multithread = nullptr;
    R_CHK(pDevice->QueryInterface(IID_PPV_ARGS(&multithread)));
    if (multithread) {
