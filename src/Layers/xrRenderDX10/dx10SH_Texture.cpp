...
void CTexture::apply_seq(u32 dwStage)
{
	...
	if (flags.seqCycles)
	{
		u32 frame_id = frame + frame_data > 0 ? frame % (frame_data * 2) : 0;
		if (frame_id >= frame_data && frame_id + frame_data > 0) frame_id = (frame_data - 1) - (frame_id % frame_data);
		pSurface = seqDATA[frame_id];
		m_pSRView = m_seqSRView[frame_id];
	}
	else
	{
		u32 frame_id = frame+frame_data>0 ? frame % frame_data : 0;
		pSurface = seqDATA[frame_id];
		m_pSRView = m_seqSRView[frame_id];
	}
	//CHK_DX(HW.pDevice->SetTexture(dwStage,pSurface));
