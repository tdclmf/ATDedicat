...
XRSOUND_API extern xrSRWLock sound_core_lock;
// Flags
enum
{
	ss_Hardware = (1ul << 1ul),
...
public:
	...
	IC CSound_source* _handle() const { xrSRWLockGuard guard(&sound_core_lock, true); return _p ? _p->handle : NULL; }
	IC CSound_emitter* _feedback() { xrSRWLockGuard guard(&sound_core_lock, true); return _p ? _p->feedback : 0; }
	IC CObject* _g_object()
	{
		xrSRWLockGuard guard(&sound_core_lock, true);
		VERIFY(_p);
		return _p->g_object;
	}
	...
	IC float get_length_sec() const { xrSRWLockGuard guard(&sound_core_lock, true); return _p ? _p->get_length_sec() : 0.0f; };
};
...
public:
	...
	virtual concurrency::task_group& sound_loader() = 0;
...
{
	xrSRWLockGuard guard(&sound_core_lock, false);
...
{
	if (_feedback()) _feedback()->stop(::Sound->i_locked());
}
