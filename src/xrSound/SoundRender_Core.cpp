...
{
	PROF_EVENT("CSoundRender_Core::create");
	if (!bPresent) return;
...
void CSoundRender_Core::attach_tail(ref_sound& S, const char* fName)
{
	...
	if (S._p->feedback)
		((CSoundRender_Emitter*)S._p->feedback)->fTimeToStop += s->length_sec();
...
{
	if (!bPresent || 0 == S._p || 0 == S._p->handle)return;
	S._p->g_object = O;
	if (S._p->feedback) ((CSoundRender_Emitter*)S._p->feedback)->rewind();
	else i_play(&S, flags & sm_Looped, delay);

	if (flags & sm_2D || S._p->handle->channels_num() == 2)
		S._p->feedback->switch_to_2D();
}
                                         ...
                                         float* freq, Fvector2* range)
{
	...
	if (pos) S._p->feedback->set_position(*pos);
	if (freq) S._p->feedback->set_frequency(*freq);
	if (range) S._p->feedback->set_range((*range)[0], (*range)[1]);
	if (vol) S._p->feedback->set_volume(*vol);
	S._p = orig;
}
...
void CSoundRender_Core::play_at_pos(ref_sound& S, CObject* O, const Fvector& pos, u32 flags, float delay)
{
	...
	S._p->feedback->set_position(pos);
...
{
	if (S._p && S._p->feedback)
	{
		CSoundRender_Emitter* E = (CSoundRender_Emitter*)S._p->feedback;
		E->stop(FALSE);
	}
	S._p = 0;
}
