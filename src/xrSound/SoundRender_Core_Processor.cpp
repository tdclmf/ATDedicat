...
extern xrCriticalSection emitters_lock;
void CSoundRender_Core::update(const Fvector& P, const Fvector& D, const Fvector& N)
{
	PROF_EVENT("CSoundRender_Core::update");
	xrSRWLockGuard guardsrw(&sound_core_lock, true);
	xrCriticalSection::raii guardcs(&emitters_lock);
	u32 it;
	...
	if (!s_targets_defer.empty())
	{
		...
		{
			CSoundRender_Target* target = s_targets_defer[it];
			if (!target->get_emitter()) continue;
...
{
	static thread_local CDB::COLLIDER xrc;
	float occ_value = 1.f;
	...
	if (0 != geom_SOM)
	{
		...
		xrc.ray_query(geom_SOM, hear_pt, dir, range);
		u32 r_cnt = xrc.r_count();
		CDB::RESULT* _B = xrc.r_begin();
#endif
		if (0 != r_cnt)
		{
...
float CSoundRender_Core::get_occlusion(Fvector& P, float R, Fvector* occ)
{
	...
	if (0 != geom_MODEL)
	{
		...
		if (bNeedFullTest)
		{
			...
			xrc.ray_query(geom_MODEL, base, dir, range);
			if (0 != xrc.r_count())
			{
				// cache polygon
...
#else
		xrc.ray_options(CDB::OPT_CULL);
		xrc.ray_query(geom_SOM, base, dir, range);
		u32 r_cnt = xrc.r_count();
		CDB::RESULT* _B = xrc.r_begin();
#endif
		if (0 != r_cnt)
