...
{
	if(GetCurrentThreadId() != Device.mmThreadID && GetCurrentThreadId() != Device.uiThreadID && spatial.space != g_SpatialSpacePhysic)
		Debug.ShowStackTrace("Stack Trace", true);
...
void ISpatial::spatial_register()
{
	...
	else
	{
		...
		xrSRWLockGuard guard(spatial.space->bvh_lock, false);
...
{
	Fvector curr_sector_point = spatial_sector_point();

	if ((FALSE == curr_sector_point.similar(last_sector_point) || spatial.sector == nullptr))
	{
		PROF_EVENT("spatial_updatesector");
		spatial.type &= ~STYPEFLAG_INVALIDSECTOR;
		if (IRender_Sector* S = ::Render->detectSector(spatial_sector_point()))
			spatial.sector = S;
	}

	last_sector_point = curr_sector_point;
}
...
void ISpatial_DB::remove(ISpatial* S)
{
	...
	stat_remove.Begin	();
#endif
	if(ISpatial_NODE* N = S->spatial.node_ptr)
	{
		N->_remove(S);
