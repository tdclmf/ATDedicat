...
static constexpr u32 kRemoteDropNetSyncBlockMs = 3000;
...
CInventoryItem::CInventoryItem()
{
	...
	m_remote_drop_net_sync_block_until = 0;
...
#endif
	if (OnClient()) // werasik2aa is it need?
		Interpolate();
}
...
void CInventoryItem::net_Import(NET_Packet& P)
{
	...
	const bool dedicated_single_remote_drop =
		OnClient() &&
		!g_dedicated_server &&
		!this->cast_game_object()->Local() &&
		!object().H_Parent();
	const bool remote_drop_sync_blocked =
		dedicated_single_remote_drop &&
		(Device.dwTimeGlobal < m_remote_drop_net_sync_block_until);
	// Dedicated-single bridge:
	// right after authoritative drop we intentionally keep remote item on local shell
	// simulation for a short window to avoid stale rest packets pinning item in air.
	if (remote_drop_sync_blocked)
	{
		if (object().m_pPhysicsShell && !object().m_pPhysicsShell->isActive())
			object().m_pPhysicsShell->Activate(false);
		WPN_TRACE("InventoryItem::net_Import ignore remote drop sync item=%u remain_ms=%u",
			object().ID(), m_remote_drop_net_sync_block_until - Device.dwTimeGlobal);
		return;
	}
	const bool dedicated_single_remote_drop_active_shell =
		dedicated_single_remote_drop &&
		object().m_pPhysicsShell &&
		object().m_pPhysicsShell->isActive();
	// Dedicated-single bridge:
	// non-owner clients can occasionally receive stale/static state right after authoritative drop.
	// If object is already moving under local shell simulation, ignore that stale packet.
	if (dedicated_single_remote_drop_active_shell)
	{
		Fvector current_velocity;
		object().m_pPhysicsShell->get_LinearVel(current_velocity);
		const bool currently_moving = current_velocity.square_magnitude() > (0.20f * 0.20f);
		const bool incoming_static = N.State.linear_vel.square_magnitude() <= (0.02f * 0.02f);
		const bool incoming_near_current = N.State.position.distance_to_sqr(object().Position()) <= (0.03f * 0.03f);
		if (currently_moving && incoming_static && incoming_near_current)
		{
			WPN_TRACE("InventoryItem::net_Import skip stale remote drop state item=%u pos=(%.3f,%.3f,%.3f) in_vel=(%.3f,%.3f,%.3f) cur_vel=(%.3f,%.3f,%.3f)",
				object().ID(),
				N.State.position.x, N.State.position.y, N.State.position.z,
				N.State.linear_vel.x, N.State.linear_vel.y, N.State.linear_vel.z,
...
void CInventoryItem::Interpolate()
{
	...
	const bool remote_drop_sync_blocked =
		dedicated_single_remote_drop &&
		(Device.dwTimeGlobal < m_remote_drop_net_sync_block_until);
	if (remote_drop_sync_blocked && object().m_pPhysicsShell && !object().m_pPhysicsShell->isActive())
		object().m_pPhysicsShell->Activate(false);
	// Dedicated-single bridge:
	// for remote dropped items we already apply authoritative drop pos/vel in GE_OWNERSHIP_REJECT.
	// Further network interpolation may overwrite local shell simulation and cause hovering.
	if (remote_drop_sync_blocked ||
		(dedicated_single_remote_drop && object().m_pPhysicsShell && object().m_pPhysicsShell->isActive()))
	{
		if (!p->NET_IItem.empty())
		{
			WPN_TRACE("InventoryItem::Interpolate bypass remote drop item=%u queue=%u", object().ID(), (u32)p->NET_IItem.size());
			p->NET_IItem.clear();
		}
		return;
	}
...
void CInventoryItem::ClearNetInterpolationQueue()
{
	if (!m_net_updateData)
	{
		m_remote_drop_net_sync_block_until = Device.dwTimeGlobal + kRemoteDropNetSyncBlockMs;
		return;
	}
	m_net_updateData->NET_IItem.clear();
	m_remote_drop_net_sync_block_until = Device.dwTimeGlobal + kRemoteDropNetSyncBlockMs;
}
