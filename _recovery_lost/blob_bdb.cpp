...
void CActor::net_Export(NET_Packet& P) // export to server
{
	...
	if (H_Parent() || OnServer() || NumItems > 1 && OnClient())
		NumItems = 0;
...
BOOL CActor::net_Spawn(CSE_Abstract* DC)
{
	...
	{
		Msg("--- [SV] Dedicated single: CActor::net_Spawn skipped for ID: %u", E->ID);
		return FALSE;
	}
	...
	// 3. ???????? ?? ????????:
	// ????????? ?????? ???????? SetEntity. ?????? ???????? ?????? ????? ??? ???????.
	if (!g_dedicated_server && E->s_flags.is(M_SPAWN_OBJECT_ASPLAYER))
	{
		Msg("--- [CL] Possession SUCCESS for ID: %u", ID());
		Level().SetEntity(this);
	}
	else
	{
		Msg("--- [SV] Possession SKIPPED (Dedicated server or Remote puppet)");
	}
	...
	m_pPhysics_support->in_NetSpawn(DC);
	...
	if (!g_dedicated_server && E->s_flags.is(M_SPAWN_OBJECT_LOCAL) && E->s_flags.is(M_SPAWN_OBJECT_ASPLAYER))
...
void CActor::OnChangeVisual(bool destroy)
{
	{
...
{
	bool a = inherited::BonePassBullet(boneID);
	// werasik2aa not sure
	...
	if (!pOutfit)
	{
		...
		return bone_instance.get_param(3) > 0.5f || a;
	}
	return pOutfit->BonePassBullet(boneID) || a;
}
